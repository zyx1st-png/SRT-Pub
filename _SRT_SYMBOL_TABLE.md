...
| **h[A]** | `h[A]` | Temporal Entropy Rate of Schedule | h[A] ≡ lim_{T→∞} H(A(0), A(Δt), …, A(T))/(T/Δt); Shannon entropy rate. | Bits/time | Core; canonical source `SRT_Core_14 T-Scale-Rhythm-3` |
| **τ_off^*** | `\tau_{off}^{*}` | Optimal Off-Phase Duration | Optimal off-phase length balancing entropy-export benefit against noise-erosion cost. | Time | Core; canonical source `SRT_Core_14 T-Scale-Rhythm-4` |
| **c_B** | `c_B` | Bandwidth-d Proportionality Constant | Positive constant (c_B > 0) in B_θ = c_B · d. | Hz (per unit d) | Core; canonical source `SRT_Core_14 Ax-Spec-01, Cor-Scale-Rhythm-4a` |
| **ρ_I** | `\rho_I` | Information Density Lower Bound | Positive constant (ρ_I > 0) in conditional strengthening of Cor-Scale-Rhythm-4a. | Bits/(Hz·time) | Core; canonical source `SRT_Core_14 Cor-Scale-Rhythm-4a` |
| **k_n** | `k_n` | Sub-Targets per Layer | Model-local number of anchoring sub-targets. | Integer (≥ 1) | Core; conditional source `SRT_Core_14 T-Scale-Rhythm-5` |
| **τ_switch^min** | `\tau_{switch}^{min}` | Minimum Feasible Switch Time | Model-local lower bound on per-switch duration. | Time | Core; conditional source `SRT_Core_14 T-Scale-Rhythm-5` |
| **σ_{sr}** | `\sigma_{sr}` | Self-Reference Ratio | `σ_{sr} := ‖θ^{trace}‖ / (‖θ^{trace}‖ + ‖θ^{ext}‖) ∈ [0, 1]`. **P2 canonical interpretation:** `SRT_Individuation.md §八` already states that the order-parameter choice is conventional and replaceable by an equivalent form. In the current bare-norm realization it is a model-local historical/writeback-balance proxy under a declared trace/ext attribution rule, parameter representation, and norm/metric. | Scalar [0,1] | L1; canonical source `Core_Law/SRT_Individuation.md` and `Core_Law/SRT_L1_Formalism.md §2`. Not by itself a representation-invariant natural quantity, unique history-attribution theorem, causal-control share, bearer/selector criterion, or proof of a natural phase boundary. Distinct from main-equation state field `σ` and anchoring sub-target `σ_j`. |
| **σ_{sr}^{sub}** | `\sigma_{sr}^{sub}` | Subject-Position Entry Threshold | First model-coordinate threshold used by the current individuation parameterization for subject-position entry. Its existence/placement is a P1-candidate/P2 strong-candidate **model-class interpretation**; concrete value P3/P4. | Scalar | L1 structural hypothesis; does not by itself establish that this particular coordinate is a representation-independent natural phase boundary. The broader question whether P1-T06 has an independently evidenced natural transition boundary remains open. Source `SRT_Individuation.md T-IND-2, §七.8, §八`. |
| **σ_{sr}^{self}** | `\sigma_{sr}^{self}` | Self-Consciousness Condensation Threshold | Second model-coordinate threshold used by the current σ parameterization for second-order writeback condensation. Higher than `σ_{sr}^{sub}` inside the declared model; concrete value P3/P4. | Scalar | L1 structural hypothesis / P2 strong candidate in the current parameterization. Does not make the bare σ coordinate natural or representation-invariant; phenomenality remains separately open. Source `SRT_Individuation.md T-IND-3, §八`. |
| **σ_{sr}^{health}** | `\sigma_{sr}^{health}` | Healthy Operating-Point Center | Model-local operating-point center within the declared σ dynamics, used to track balanced self-reference without pathological closure toward `σ_{sr} → 1`. | Scalar | L1 structural hypothesis / operating proxy; specific value P3/P4 and representation/model-class conditional. It is not a universal natural constant or standalone health criterion. Source `SRT_L1_Formalism.md §2.4, §5.3`. |
| **σ_{sr}^{coll}** | `\sigma_{sr}^{coll}` | Collective Self-Reference Ratio | Multi-ISP extension over shared `L_2` field `\mathcal{P}`. In the current owner it includes **boundary-relative reclassification**: shared `L_2` is moved into collective trace because it is no longer new input relative to the declared collective exterior. | Scalar [0,1] | L1 model-local collective historical/endogeneity proxy under declared collective boundary, attribution, representation and norm. Its trace semantics are not automatically identical to individual own-output provenance; `σ_{sr}^{coll}→1` remains a model-class pathological-collapse indicator, not a representation-independent natural constant, collective-agency criterion, or causal-control share. Source `SRT_Collective_Selection.md §4.1, §4.4, §4.7`. |
...
*Reading rule*: a Tier 3 proxy or threshold must never be cited as if it were the Tier 1 anchor it approximates. Exact claim level remains source-local; cross-domain ranking of subjecthood, concern, or consciousness from a Tier 3 readout is out of scope for this table.
...
12. **σ namespace + scope split** (2026-04-24 namespace; 2026-08-29 S+R truth-up): bare `σ` defaults to the **main-equation state field** (`Core/SRT_Core_22_Equations.md`); `σ_{sr}` is the self-reference-ratio family; `σ_j` is an anchoring sub-target. These are three different objects. The `σ_{sr}` family inherits `SRT_Individuation.md §八`: its order-parameter choice is P2, conventional, and replaceable by an equivalent form. The current bare-norm realization is valid only under a declared trace/ext attribution rule, parameter representation and norm/metric. Do **not** infer from namespace registration that `σ_{sr}` or `σ_{sr}^{coll}` is representation-invariant, a unique provenance decomposition, a causal-control share, a bearer/selector criterion, or a natural phase coordinate. `σ_{sr}^{coll}` is additionally boundary-relative and may reclassify shared `L_2` as collective trace. Thresholds `sub/self/health` are model-class coordinates until stronger invariant/natural-boundary evidence is separately established. Historical files using bare `σ` for self-reference are read as `σ_{sr}` only after applying this scope rule.
13. **Governance terms are not theory symbols**: `interface_annex`, `copy-to-annex`, `owner-bound`, `claim_mode`, `canonical:false`, and related structural-governance vocabulary are repository-organisation terms, not canonical mathematical or phenomenological symbols.
14. **Annex and Operations reference scope**: Non-canonical annex files and Operations records may cite and use symbols defined in this table. They must not introduce new symbol definitions, override existing usage rules, or alter canonical scope.
15. **κ namespace split** (GOV-SUB01 Pass 1, 2026-07-16): the glyph `κ` spans two unrelated object families that must not be read as one continuous quantity. (a) `κ_0` and `κ(t)` are L₀-curvature objects. (b) `κ_{c1}` and `κ_{c1.5}` are consciousness-stage phase-transition thresholds. The two families share no derivation path. This rule adds a namespace guard only and does not modify any equation.
16. **Gate 0 — L₀ contentless structurality boundary** (adopted 2026-08-11): pointer only. Whether a given object may sit at bare `L_0` is decided solely by `Governance/SRT_CLAIM_LADDER.md §0A`.
17. **B-A — Shoshin strict-layering guard** (author-decided 2026-08-12): this decision deliberately adds no symbol. `ε_pg` remains the registered L₀ minimum-non-neutrality postulate and must not be renamed or aliased as a Shoshin precursor.
18. **C-A — global-optimum scope guard** (author-decided 2026-08-12): this decision adds no symbol and does not repurpose `\Omega`. Model-local optima require declared domains and failure conditions; universe-wide semantic/evaluative optimum is not a current positive SRT object.
