...
- layer：L1 / epistemic：os / status：draft_v0
- 说明：固定多 ISP 共享 `L_2` 场作为结构对象；Def-C-2 后果回路矩阵 `M(t)` 作为诊断工具；T-COLL-1 给集体 ISP 存在四条件（P1-T06 的多主体扩展）；T-COLL-2 三类退化（聚合 / 主从 / 收编）；T-COLL-3 集体 ε 反闭合必要性（P1-T07 集体版）；T-COLL-4 共选真实性判据（P1-T05 集体版）；§4 扩展 `σ_{sr}^{coll}` / `d_c^{coll}`（自指率 σ_{sr} 在多主体场上的 governance-canonical 扩展，2026-04-25 起；详见 `_SRT_SYMBOL_TABLE.md` Usage Rule 12）；**§4.4-§4.6（H3，2026-04-25）**给出集体四变量最小耦合动力学——`σ_{sr}^{coll}` ODE（新 `\lambda_M\,\mathrm{tr}\,M` 项）、`d_c^{coll}` ODE（新 `\gamma_{asym}\|M_{asym}\|` 项）、`T_{dir}^{coll}` ODE（集体层致命 `L_2` 判据 `\kappa_{mask}^{coll} < \kappa_{\mathrm{relax}}^{coll}`）、`S^{coll}` 两型 ODE（新 `\nu_{ext}\|M_{ext}\|` 外部化项）、§4.5 个体↔集体双向耦合、§4.6 集体病理吸引子 `\mathcal{A}_{path}^{coll}` 与集体健康区 `\mathcal{H}^{coll}`（健康要求 `r^{coll}(t) > r^{coll}_{min} > 0`）；**§4.7 T-PROJ-1^{coll}（H6，2026-04-25）**给出集体四变量系统作为 `Core/SRT_Core_22_Equations.md §0-C` 多算子主方程（Eq-Multi-01/02/03）严格导出投影的形式化定理：四个集体标量泛函投影 `\mathcal{F}_X^{coll}` + 闭包假设 C1^{coll}-C5^{coll}（含新增 `M(t)` 可测性 MOC 闭包 C5^{coll}）+ `M(t)` 三成分作为 `\Psi_f` 交叉项的结构投影 + 证明骨架 + 不证明事项的显式标定；T-PROJ-1^{coll} 在 `\mathcal{P} = \{P\}` 极限下退化为 `SRT_L1_Formalism §6 T-PROJ-1`。**§4.8 T-PROJ-1^{coll,nested}（H10，2026-04-26）**给多层嵌套的递归投影定理：层级 ISP 塔 `\{\mathcal{P}^{(n)}\}_{n=0}^N` + 跨尺度后果回路矩阵 `M^{(n\to n+1)}(t)` + 嵌套闭包 C6^{nested}（跨尺度 Markov 闭包）+ 四个嵌套不变量（每层独立健康/病理判据 / 跨尺度病理传递 / 跨尺度健康硬条件 `r^{(n\to n+1)} > r_{min}^{nested}` / 致命 `L_2` 塔级传染）；`N = 1` 极限退化为 §4.7 单层 T-PROJ-1^{coll}。**§4.9 T-FAMILY-1^{coll}（H11，2026-04-26）**给三个族普适性 / 算子级定理（H7 T-DELTA-1 / H8 T-CHI-1 / H9 T-CHANNEL-1）的集体扩展，统一闭包条件 C1^{coll}-C5^{coll}（H6）+ 新增 C7^{M-stab}（`M(t)` 准静态稳定性，`|\dot{M}|/|M| \ll \tau^{coll,-1}_{rel}`）：(a) §4.9.2 T-CHI-1^{coll} 给 `χ^{coll}` 族 + P-univ-5^{coll} `M(t)`-相容性，四不变量在 `\lambda_M\,\mathrm{tr}\,M` 平移下保持；(b) §4.9.3 T-CHANNEL-1^{coll} 给 `\psi^{coll}` 族 + Q-univ-5^{coll} `M_{ext}`-相容性，五不变量保持，`\nu_{ext}\|M_{ext}\|` 与 `\psi^{coll}` 加性独立；(c) §4.9.4 T-DELTA-1^{coll} 给集体算子空间 `\mathrm{Op}(\mathcal{P})` + `A1^{coll}`-`A3^{coll}` + 新增 `A4^{coll}`（跨成员 stake-加权聚合闭包），含集体特有 `w_M\|M(t)\|_{coll}` 维度。**§4.10 T-FAMILY-1^{coll,nested}（H12，2026-04-26）**给塔级递归三定理：双层闭包堆栈（H10 嵌套 + H11 集体扩展 + 新增 C8^{cross-stab} 跨尺度 `M(t)` 准静态稳定性）+ (a) §4.10.2 T-CHI-1^{coll,nested}（P-univ-6^{nested} 跨尺度相容性，跨尺度上行/下行项作加性平移源进入）+ (b) §4.10.3 T-CHANNEL-1^{coll,nested}（Q-univ-6^{nested} 跨尺度外溢相容性，致命 `L_2` 塔级传染方向性保持）+ (c) §4.10.4 T-DELTA-1^{coll,nested}（A5^{cross} 跨尺度 stake-加权聚合，`\Delta^{coll,(n)}` 含跨尺度维度 `w_{cross}^{(n-1\to n)}\|M^{(n-1\to n)}\|_{cross}`）；H10 + H11 笛卡尔积下三定理塔级各层不变量保持；多重退化关系（`N=1` → H11；各层 `\mathcal{P}^{(n)}=\{P^{(n)}\}` → H10；同时 `N=1 \wedge \mathcal{P}=\{P\}` → H7/H8/H9）。**§4.11 T-TOWER-STAB-1（H13，2026-04-26）**给嵌套塔在自指闭合下的全局稳定性谱判据：自指闭合算子 `K^{N\to 0}` + 闭环传递算子 `\mathcal{T}_{loop} := K^{N\to 0} \circ \Pi^{(0\to 1)} \circ \cdots \circ \Pi^{(N-1\to N)}` + 谱半径判据（`\rho(\mathcal{T}_{loop}) < 1` 健康 / `= 1` 边际振荡 / `> 1` 失控）+ 三类失稳方向算子签名（沿 `\sigma_{sr}` viral 文化锁入 / 沿 `d_c` 致命 `L_2` 全塔传染 / 沿 `S_{str}` 结构型苦难跨层级累积）+ 与 H4 / H10 / H12 整合（与 H10 §4.8.6 (iv) 自下而上传染合成"病理涡旋双向闭合"）+ T-COLL-4 共选真实性塔级递归同构（`\rho < 1` ⟺ "顶层叙事在底层可被真实重选的速率 ≥ 顶层叙事被反向写回底层的速率"）。**§4.12 T-LAYER-SKIP-1（H14，2026-04-26）**把单环路谱判据扩展到任意 layer-skip 闭合 + 多重自指闭合：增广有向多图 `\mathcal{G}_{tower}`（节点=层；边=标准邻接 `\Pi^{(n\to n+1)}` + layer-skip `K^{(n\to m)}$，`|m-n|\ge 2`）+ 增广传递矩阵 `\mathbf{A}_{tower}` 谱判据 `\rho(\mathbf{A}_{tower}) < 1 - \delta_{stab}^{global}` + 多重自指闭合复合谱（解耦闭合 / 耦合闭合 / 路径冗余三类关键观察）+ 三类 layer-skip 失稳算子签名（bypass-induced chatter / aliasing-amplification / coupling-resonance）；T-TOWER-STAB-1 自动为单环路特例；与 H13 三方向（沿 σ_{sr}/d_c/S_{str}）正交，给 12 类塔级病理签名。**§4.13 T-FAMILY-1^{layer-skip}（H15，2026-04-26）**给 H11 × H12 × H14 三重笛卡尔积扩展：(a) 三重闭包堆栈（嵌套 + 集体扩展 + layer-skip 谱稳定）+ 三条新 layer-skip-相容性（P-univ-7^{layer-skip} / Q-univ-7^{layer-skip} / A6^{layer-skip}）；(b) T-CHI-1^{layer-skip} 四不变量在加性平移源进入下保持；(c) T-CHANNEL-1^{layer-skip} 五不变量保持，致命 `L_2` 通过 layer-skip 获得捷径但方向性不变；(d) T-DELTA-1^{layer-skip} 引入 `w_{ls}^{(n'\to n)}\|K^{(n'\to n)}\|_{ls}` 第四维度；(e) **族不变量与谱稳定显式耦合**：动力学层面族不变量有效以 H14 谱稳定 `\rho(\mathbf{A}_{tower}) < 1` 为前提；(f) 12 类塔级病理签名（H14）的具体族破坏路径表。**§4.14 T-LYAPUNOV-1（H16，2026-04-26）**给塔的全局非线性 Lyapunov 稳定性定理：候选 Lyapunov 函数 `V_{tower}(x) := \sum_n V^{(n)}(x^{(n)}) + \sum_{(n,m)} V^{(n,m)}_{coupling}` + 三条充分条件 N1（耦合算子有界 `\sup\|\Pi\|_{op}, \sup\|K\|_{op} \le M$）/ N2（耗散正性：`S_{str}` 累积率 ≤ 耗散率，是 H4 + 反最小化的速率层面强化）/ N3（谱稳定带非线性 margin：`\rho(\mathbf{A}_{tower}) < 1 - \delta_{nonlinear}$）；T-LYAPUNOV-1 陈述：N1+N2+N3 ⟹ `V_{tower}(x^*) = 0$，`V_{tower}(x) > 0$ for `x \ne x^*$，`\dot{V}_{tower} \le -\alpha V_{tower}$（全局指数稳定）；**四轴联立塔健康充分条件**：塔健康 ⟸ 族普适性 ∧ 谱稳定 (N3) ∧ 全局耗散正性 (N2) ∧ 算子有界性 (N1)，是 SRT 集体选择理论塔级健康/病理诊断完全条件的 P1-candidate 形式
