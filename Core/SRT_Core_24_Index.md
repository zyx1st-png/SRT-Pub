...
## 2. When to Retrieve Core 24

Retrieve Core 24 when the query mentions any of the following:

- SRT 的解释力为什么强；
- SRT 是否是“万能解释”；
- object-first ontology / 对象优先本体论；
- 错误地板 / floor replacement / 地板替换；
- selection-first ontology / 选择优先本体论；
- 选择先于存在的外部防御；
- 目的、价值、道德、框架的动力学化；
...
### Phase 0 — Supplement and indexing
...
- [x] `Core/SRT_Core_21_Minimal_Axioms.md`: selection-first framing note inserted as a non-axiom framing note.
- [x] `Core/SRT_Core_12b_Ontology_L2.md`: L₂ hardening operational signature reported inserted between `T-L2-01` and `T-L2-Scaffold`.
- [x] `SRT_EXP_MEASURE_MAP.md`: non-reductive validation rule reported inserted before `## 5) 使用原则`.
- [x] Addenda preserved for both long-file integrations.

### Phase 2 — Canonical Bridge Integration

- [x] `Core/SRT_OPEN_TENSIONS.md`: Core 24 failure conditions inserted.
- [x] `_SRT_D_VALUE_CANONICAL.md`: value-as-non-substitutability bridge reported inserted after `Def-d-canonical` and before `§2b`.
...
## 4. Integrated Claims by Target File

| Claim | Target file | Status | Boundary |
|---|---|---|---|
| Selection-first / floor replacement framing | `Core/SRT_Core_21_Minimal_Axioms.md` | integrated as non-axiom framing | Does not create a new P0 primitive axiom. |
| L₂ hardening operational signature | `Core/SRT_Core_12b_Ontology_L2.md` | reported integrated | Does not define the whole ontology of `L_2`; only an operational bridge. |
| Non-reductive validation rule | `SRT_EXP_MEASURE_MAP.md` | reported integrated | Proxies do not become ontology. |
...
> Core 24 proves purpose, value, morality, and frameworks are fully formalized.
...
## 7. Future Canonical Promotion Candidates

The following lines are candidates for future higher-level promotion if supported by additional formalization or empirical discrimination.

### Candidate 1 — Floor Replacement
...
### Candidate 2 — Dynamic Normativity
...
### Candidate 3 — Non-Reductive Verification
...
### Candidate 4 — L₂ Hardening

> L₂ hardening can be operationally recognized as reduced local selection cost, increased global constraint, and hysteresis under perturbation.
