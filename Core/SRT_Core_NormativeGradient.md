...
- d 值高：规范梯度从多维度整合读取 → 读数更完整、更不易陷入局部最优陷阱（「更接近全局自由能最小值」的强读法依赖尚待硬化的闭包边界问题，按 `_SRT_D_VALUE_CANONICAL.md §5b.1` 2026-07-05 规范性收口读取）

**Cross-ref**: `_SRT_D_VALUE_CANONICAL.md §5b.1`（d扩张与全局最优读法的收口）。
