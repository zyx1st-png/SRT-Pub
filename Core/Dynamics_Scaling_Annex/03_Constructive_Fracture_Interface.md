---
id: SRT-CORE-14-ANNEX-03-CONSTRUCTIVE-FRACTURE
type: dynamics
...
> **WTC-127 consciousness guard**：本 annex 中旧“三条件”、`d / Ψ_f / \hat G` 门槛、具身必要性与 anti-panpsychism 缺席判定仅作 historical / superseded 来源。这些量只能在声明模型中作候选证据；不建立 consciousness / phenomenality / subjecthood，精确充分性保持 OPEN。
...
### Eq-Auto-01: Semantic Boundary Maintenance

θ 的时间演化由本体论摩擦梯度与预测误差学习的拮抗决定：

$$\frac{d\theta}{dt} = -\alpha \nabla_\theta \Psi_f + \beta \cdot \varepsilon_t$$

- $-\alpha \nabla_\theta \Psi_f$：摩擦梯度项，将 θ 拉向摩擦减小方向，维持现有自我边界（保守力）；$\alpha$ 为摩擦权重系数
- $\beta \cdot \varepsilon_t$：学习项，$\varepsilon_t$ 为时刻 $t$ 的预测误差，$\beta$ 为学习率；新经验与预期的偏差驱动 θ 更新

**稳定自我态**：当 $\alpha \nabla_\theta \Psi_f = \beta \cdot \varepsilon_t$ 时 $\frac{d\theta}{dt} = 0$，稳定性取决于 $\Psi_f$ 的曲率——势井越深（曲率越高）自我结构越稳固，势井越浅则越易被新经验扰动。

自我的连续性不是固定本质，而是摩擦阻力与学习驱动力在 θ 空间中动态平衡的轨迹。
