---
id: SRT-CORE-14-ANNEX-00-GENERAL-BOUNDARY
type: dynamics
...
**SRT扩展目标** (含利他项):
$$F_{SRT} = F_{base} - d \cdot U_{\text{others}}, \quad F_{base}\in\{F_{thermo}, F_{var}\}$$

#### 1.3.2 d值效应的显著性

**低d** ($d \approx 1$):
$$F_{SRT} \approx F_{base} \quad \text{(他者项近似不进入目标)}$$

最小化$F_{SRT}$ → 在既定基线目标上近似退化为自我维持。

**高d** ($d \gg 1$，且 $d \cdot U_{\text{others}}$ 成为主导目标项):
$$F_{SRT} \approx F_{base} - d \cdot U_{\text{others}} \quad \text{(优化方向由关切项主导)}$$

最小化$F_{SRT}$ → 在维持自身基线约束的前提下优先保障他者福祉（必要时可出现自我牺牲）。
