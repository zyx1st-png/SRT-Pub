...
| $\hat{G}_\theta$ | Ghost Operator | primitive actualisation 的形式角色：参数化选择映射 $L_0 \to L_1$；不是先在实体或自身原因 |
...
#### 1.1.1 Ruliad 与模空间的双重投影

$L_0^{abs}$ 并非一个可由单一数学对象完备定义的“可能性集合”。以下两种对象只能作为特定 $L_2$ 框架下的形式投影：
...
2. **计算理论视角 (Ruliad)**:
   - Ruliad 形式化所有可能计算规则及其演化轨迹的极限对象；它不是 $L_0^{abs}$ 本身
   - Ruliad 只能作为特定 $L_2$ 框架下的形式投影，不能据此把 $L_0^{abs}$ 读成完成形式库存；创新受已有约束与潜在条件限制，但不必是对早已完成之对象的发现
...
**哲学意义**：创新不是无约束的“从无到有”，但也不是对 $L_0$ 中固定照明份额的重新分配。新确定形式受历史与约束生成，不必预先作为同一完成对象存在；上式若继续使用，只能是指定相对状态空间中的局部记账类比。
...
其中 $\pi_\lambda$ 是具名粗粒化映射。该式只有在状态空间、保留观测量、比较范数、容差与失败例均已声明时才可检验；通过只建立局部可比性，不证明普遍同构或机制同一。

> **注（P3-B06／P3-B07）**：以下各尺度现象可接受“是否共享选择—约束—可支付性语法”的比较。是否具有共同动力学须逐尺度对通过近似交换与判别增益检验；不得从形式相似直接写成同一结构。详见 `Core/SRT_Core_21c_Bridge_Hypotheses.md P3-B06/P3-B07`。
>
> **注（Ax-F-12 / P2-P3-B08 对应）**：$\Psi_f$ 在此不只表示锚定代价，也表示动力学生成来源。演化、学习、文化变迁等过程都可理解为算子间本体论摩擦的不同尺度形态；没有 $\Psi_f$ 就没有现实生成。详见 `Core/SRT_Core_21c_Bridge_Hypotheses.md P2/P3-B08`。
...
| 符号 | 名称 | 定义位置 | 意义 |
|:-----|:-----|:---------|:-----|
| $L_0$ | 潜在域 | §1.1 | 不可被有限显现穷尽的结构化潜在；Ruliad / Moduli Space 为领域投影 |
