...
**B-A 后的 SRT 答案**：目标不是一个已在 $L_0$ 中携带语义内容、再被 $L_2$ 修正的 `Target_0`。$L_0$ 只提供无内容结构边界；可操作的目标由有限算子的具身需求、当前位置、历史约束、可达候选与后果回流在 L₁/L₂ 中共同形成。初心可以是 L₁/P2 对这种处境化方向的回读，但不是目标内容的 L₀ 来源。

结构输入可压缩写为：`Target(t)` 取决于具身状态、当前 `L_1`、历史 `L_2`、当下可达候选与后果回流；这里不再为它引入一个普遍的 `Target_0` 或新的 canonical 函数符号。

**实例**:
- 具身需求与当前状态：饥饿使食物相关候选获得局部优先级；这不是 L₀ 语义目标
- $L_2$ 历史／文化约束：把食物候选进一步收窄为特定料理，并可被后续经验改写

这只是算子层的结构输入清单，不是通用目标生成定理；具体模型必须声明各输入如何进入比较与选择。
