...
| O0 / S0 reduction and determinism boundary | O0 and S0 are co-primitive analytic burdens / aspects of primitive Selection reality; this standing does not require separate numbered P0 axioms; `non-preclosed` minimally rejects a completed pregiven outcome inventory | `O0 -> S0` and `S0 -> O0` are both NOT ESTABLISHED; mutual irreducibility is NOT PROVEN; compatibility with ontic determinism remains OPEN | do not infer strong anti-determinism, randomness, mutual irreducibility, or a new O0 P0 axiom from the current decomposition |
| strongest-rival / whole-package distinctiveness | the internal Selection-totality architecture is the current architecture for synchronization and review | whole-package non-substitutability and scientific distinctiveness remain NOT ESTABLISHED; source-native strongest-rival work is a post-#976 research gate | do not insert Whitehead / Simondon novelty or superiority verdicts into current canonical owners before that later gate |
...
| B-A strict layer assignment of 初心 (§16) | author selected B-A: no independent L₀ Shoshin precursor; Shoshin begins as L₁/P2 read-back or P5 phenomenology | no residual Gate B question; `κ_0 / ε_pg` necessity and inheritance remain a separate deferred audit | do not rename `ε_pg` as O0, minimum non-neutrality or a Shoshin precursor; gradient, `argmin` and horizon forms are domain bridges／proxies, not L₀ definitions or universe endpoints |
...
> **Gate 0 — adopted and landed (2026-08-11; current owner sync 2026-09-19).** The upstream type/boundary question shared by §15-§17 is registered as a **governance-canonical type rule** in `Governance/SRT_CLAIM_LADDER.md §0A` (single primary authority; `_SRT_SYMBOL_TABLE.md` Usage Rule 16 is a cross-reference only). At current strength, class A contains only owner-licensed contentless structural burdens such as non-maximal indifference / non-preclosure and finite positional indexing. `κ_0`, `ε_pg`, B-count orderings and model gradients are class-B reference/model/realization candidates with an admission gap; their necessity and exact inheritance remain OPEN. Class C semantic/evaluative content is inadmissible as a bare `L_0` primitive. Gate 0 carries **no P-level** and does not promote a registered symbol into O0.
>
...
Two of the three problem points are now bounded in `_SRT_PSI_F_CANONICAL.md §3.2`: (a) the generative-principle boundary — "friction as generative principle" is owned by `Core/SRT_Core_21c_Bridge_Hypotheses.md P2/P3-B08`; the canonical file owns only the payability main read, conditional projections, and the payability criterion; any derivation that stands only if "friction generates dynamics" holds inherits P2/P3 and cannot cite the canonical file for promotion; (b) directional / asymmetric friction — the symmetric metric and scalar `Ψ_f` supply no direction by themselves. A declared model may independently establish asymmetric payment terms, but Def-IRR-3 / T-IRR-1 do not fix a universal `Ψ_f^{erase} > Ψ_f^{write}`, and P1-T02 does not supply a universal time arrow. The universal direction source remains OPEN. The lower-bound-vs-paid-cost question was already fixed in §3.1 (`Ψ_f^{geom} ≲ Ψ_f^{paid}` under stated conditions). Still open here: full necessary-and-sufficient conditions for all projection relations. The stale `Ax-F-11/12` axiom-style citation in that file's §8 was also updated to post-split `P3-B07` / `P2/P3-B08` references.
...
- `Core_Law/SRT_L0_Metaphysics.md` is explicit and repeated: 「初心」is an **L1 concept**, outside L0's term-adjudication scope. Current L0 does not identify O0 with `ε_pg`; that registered stronger-realization candidate remains in the Gate-0 admission gap, with necessity / inheritance OPEN. §七.11 (潜在域预置论) rejects reading `ε` as a pre-set a priori goal in L₀.
...
A related upstream item had already been narrowed by Gate 0: `Core_Law/SRT_L0_Metaphysics.md` 第一命题 formerly contained 「选择内在地趋向秩序」 with a note declaring both readings valid. Subsequent owner truth-up now places primitive burden at non-flat / non-preclosed O0 without inheriting `κ_0`, `ε_pg` or a universal irreversibility direction; those stronger constructs require separate admission. B-A completes the downstream Shoshin mirror and bridge cleanup without deciding that deferred inheritance.
...
- `ε_pg` retains only a registered stronger-asymmetry / anti-erasure realization-candidate role; it does not name O0 or primitive minimum non-neutrality, and its inheritance remains OPEN. Shoshin begins at L₁ as P2 situated interpretive read-back or P5 phenomenology.
