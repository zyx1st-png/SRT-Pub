---
source_id: SRC-2026-08-03-PHYSICS-REAL-QUANTUM-REPRESENTATION-COMPOSITION
title: "Physicists Take the Imaginary Numbers Out of Quantum Mechanics"
source_type: science_journalism_with_primary_paper_pair_and_peer_reviewed_update
domain: Physics / Quantum foundations / Mathematical representation / Composite systems
url: https://www.quantamagazine.org/physicists-take-the-imaginary-numbers-out-of-quantum-mechanics-20251107/
doi:
  - 10.1103/4k13-sdjh
  - 10.48550/arXiv.2504.02808
  - 10.1038/s41586-021-04160-4
primary_sources:
  - title: "Quantum mechanics based on real numbers: A consistent description"
    authors: [Pedro Barrios Hita, Anton Trushechkin, Hermann Kampermann, Michael Epping, Dagmar Bruß]
    arxiv: 2503.17307
    publication: Physical Review Letters 136, 240202
    date_published: 2026-06-18
  - title: "Quantum theory does not need complex numbers"
    authors: [Timothee Hoffreumon, Mischa P. Woods]
    arxiv: 2504.02808
    status: preprint_v2
  - title: "Quantum theory based on real numbers can be experimentally falsified"
    publication: Nature 600, 625-629
    doi: 10.1038/s41586-021-04160-4
authors:
  - Daniel Garisto
publication: Quanta Magazine
date_published: 2025-11-07
date_added: 2026-08-03
evidence_level: high_quality_secondary_synthesis_plus_one_peer_reviewed_primary_paper_one_primary_preprint_and_2021_peer_reviewed_comparator
reliability_level: high_for_the_narrow_representation_and_composition_claim; medium_for_ontological_interpretation
srt_relevance: representation-covariance guard; composition-rule dependence; mathematical-scaffold humility; quantum-bridge hardening
integration_priority: A_bounded_bridge_high
related_srt_claims:
  - SRT-PHYSICS-CLAIM-STATUS
  - SRT-PHYS-P09-REPRESENTATION-COVARIANCE-COMPOSITION
  - SRT-QUANT-00-COMPACT-CORE
  - SRT-QUANT-01-COMPACT-CORE
  - SRT-L0-METAPHYSICS
tags:
  - real_quantum_mechanics
  - complex_numbers
  - representation_covariance
  - tensor_product
  - composite_systems
  - quotient_space
  - empirical_equivalence
  - quantum_foundations
  - Quanta
canonical: false
claim_mode: evidence
---

# SourceCard: real-valued quantum theory, representation covariance, and composite-system rules

## 1. One-line summary

Recent real-valued formulations reproduce the observable predictions and locality structure of standard complex quantum mechanics by changing how composite systems are represented, showing that the 2021 network experiments excluded a restricted real-theory package rather than every theory written without explicit complex numbers.

## 2. Core claims of source

- Standard quantum mechanics uses complex Hilbert spaces, and multiplication by `i` compactly represents phase and rotation structure.
- The 2021 *Nature* result compared complex quantum theory with a real-number theory that retained a particular package of postulates, including a standard composition rule for independent systems; within that package, the theories make different predictions in network experiments.
- Barrios Hita et al. replace the overly restrictive real composite-system construction with a real quotient-space formulation. Local operators still act trivially on other subsystems, and the model reproduces all multipartite quantum predictions.
- Hoffreumon and Woods distinguish the abstract tensor product from a chosen matrix representation of it. Their alternative real representation preserves representation locality and yields a real-number quantum theory experimentally indistinguishable from standard quantum theory.
- The explicit symbol `i` can therefore be removed from the formulation, but the real theory retains structures equivalent to complex phase arithmetic, including an `SO(2)` rotation/flag structure and nontrivial equivalence or combination rules.
- The narrow result is not that complex quantum mechanics is wrong. It is that empirical success does not uniquely establish the complex number field as an ontological ingredient when an empirically equivalent real presentation exists.

## 3. Evidence / method

- The Quanta article was close-read as a high-quality secondary synthesis and used to identify the relevant historical and primary-source chain.
- Barrios Hita et al., arXiv:`2503.17307`, was checked in its current v2 form. The paper was published in *Physical Review Letters* 136, 240202 on 2026-06-18. It constructs composite real systems as quotient spaces and replaces a fixed tensor-product postulate with a physical locality requirement.
- Hoffreumon and Woods, arXiv:`2504.02808v2`, was checked through its abstract and HTML text. It constructs an alternative real matrix representation of the tensor product, retains representation locality, and states that real- and complex-number quantum theories are experimentally indistinguishable within the construction.
- The 2021 *Nature* paper, DOI `10.1038/s41586-021-04160-4`, was checked as the comparator. Its own scope explicitly fixes the standard Hilbert-space postulates and notes that alternative real formulations can recover complex predictions only by violating at least one of those postulates.
- This SourceCard records the representation/composition result. It is not a full adjudication of every mathematical detail or of whether one formulation is more natural, explanatory, or fundamental.

## 4. Limits

- The sources do not prove that physical reality is fundamentally real-valued.
- They do not refute complex quantum mechanics; the complex formulation remains more compact and natural in many calculations.
- Removing explicit `i` does not remove phase, rotation, equivalence, or complex-like multiplication structure.
- Empirical equivalence under the constructions does not establish that all imaginable real and complex formulations are equivalent.
- The result does not solve the measurement problem, derive the Born rule from SRT, establish objective collapse, or decide between Copenhagen, Everett, QBism, relational, or other interpretations.
- It must not be used as empirical proof of `L_0`, `L_1`, `L_2`, `G_hat_theta`, or the claim that selection precedes stable existence.

## 5. SRT relevance

### 5.1 Representation-covariance guard

SRT physics claims should survive translation between empirically and compositionally equivalent presentations. If a proposed SRT interpretation depends only on the visible occurrence of `i`, a complex wavefunction, or one matrix realization of the tensor product, it is probably tracking notation rather than physical structure.

### 5.2 Composition rules carry substantive content

The 2021 disagreement was not determined by the number field alone. It depended on the package:

```text
local state space
+ admissible operations
+ composition rule
+ equivalence relation
+ probability/readout rule
```

This gives SRT a concrete pressure case for treating the formation of composite objects and admissible relations as part of the theory, not as a neutral afterthought.

### 5.3 Mathematical scaffold humility

Complex numbers may be the most economical `L_2` mathematical scaffold for quantum phase structure without being a literal inventory of ontological substances. The source supports a distinction between a stable representation and the cross-representation structure it compresses.

### 5.4 Quantum bridge correction

The existing shorthand `wavefunction = L_0 possibility structure` is safe only as a bounded bridge expression. A stronger and safer reading is:

```text
the wavefunction is one candidate physical representation of an accessible possibility structure;
it is not identical to absolute L_0.
```

Likewise, a selection/measurement bridge must be invariant under complex-to-real reformulation unless a separately stated empirical discriminator is supplied.

## 6. Suggested patch target

Created with this SourceCard:

- `Physics/patches/SRT_Phys_P09_Representation_Covariance_and_Composition_Guard_v0_1.md`

Future synthesis targets:

- `Physics/SRT_Physics_Bridge_v0_2.md`
- a bounded representation-dependence caution in `Physics/SRT_Quant_00_Intro.md` and `Physics/SRT_Quant_01_Selection.md` during a later controlled synthesis pass
- `Physics/SRT_Physics_Claim_Status.md` only if representation-specific language begins to be promoted beyond P3

No canonical edit is authorized by this material.

## 7. Do-not-import list

- Do not write: quantum mechanics has shown that imaginary numbers are unreal or meaningless.
- Do not write: real quantum mechanics is experimentally confirmed over complex quantum mechanics.
- Do not write: the 2021 experiments were invalid; they validly tested a restricted postulate package.
- Do not write: complex Hilbert space is `L_0`.
- Do not write: the alternative tensor-product representation proves that selection precedes existence.
- Do not identify the real flag, quotient-space equivalence class, global phase, `G_hat_theta`, or an SRT selector with one another.
