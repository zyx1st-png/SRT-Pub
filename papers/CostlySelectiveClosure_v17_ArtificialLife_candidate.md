...
The comparative gap addressed here is therefore more modest. Even when two systems can both be described as adaptive, precarious, autonomous in some sense, or well-regulated, they may differ sharply in **where maintenance costs are borne**, **how history constrains later behavior**, and **whether failure is terminal for the declared unit or cheaply externalized to surrounding infrastructure**. CSC turns those differences into one cross-system profile declaration. The empirical contribution is correspondingly narrow: a matched terminate-versus-restore intervention is paired with a lives-gradient dose-response, payoff robustness checks, and a frozen-policy common-state probe. The novelty claim is not death or reset by themselves, but this particular combination of profile protocol and controlled discrimination.
...
Nor is this paper the first artificial-agent study in which mortality or persistent consequence matters. The closest recent neighbor, Chen and Chen's (2026) Synthetic Linguistic Agency preprint, experimentally manipulates whether bodily consequences persist or are reset inside a mortality-grounded linguistic agent; their target is the causal organization of embodiment, linguistic participation, and precariousness within one continuing artificial life. CSC asks a different and broader comparison question: can support, history, and recoverability be declared as a reusable cross-substrate profile, and can one of those dimensions be isolated with a simple matched intervention whose result survives dose-response, payoff, and policy-level probes? This is a narrower novelty claim than “mortality matters,” but it is also easier to test and falsify.
