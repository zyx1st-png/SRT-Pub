...
The paper therefore makes two separable contributions. First, it proposes a particular four-question profile together with a declaration rule for unit, boundary, timescale, and recovery regime. Second, it provides a controlled experiment showing that one CSC-relevant variable, terminate-versus-restore, can materially change what an adaptive system learns.

## 2. A Comparative Gap: Consequence-Bearing Maintenance

### 2.1 Neighboring approaches already explain much of the problem

CSC is not offered as a replacement for autopoiesis, autonomy, enactivism, dimensional accounts of life, active inference, or evolutionary approaches. Each already supplies important structure.

Autopoiesis foregrounds self-production and organizational closure. Autonomy approaches develop this into richer accounts of mutually dependent constraints and self-maintenance. Enactive work makes precariousness and adaptivity explicit: regulation matters because the system's continued organization is not guaranteed. Beer and Di Paolo (2023), in particular, give precariousness a systematic treatment through systemic, processual, and thermodynamic forms of fragility. Dimensional approaches likewise predate CSC. Witkowski and Schwitzgebel (2024) explicitly recommend attending to dimensions such as regulation, homeostasis, metabolism, heredity, learning, and related properties rather than insisting on a binary boundary; Damiano and Stano (2020) argue that life-likeness assessments of synthetic cells should attend to internal organization rather than behavioral resemblance alone. Active-inference approaches formalize adaptive regulation in probabilistic and dynamical terms. Evolutionary and open-ended approaches explain persistence and innovation across lineages rather than only within individual organisms. Recent mathematical work on agents likewise emphasizes that individuality, normativity, and interaction structure must be made explicit rather than inferred from performance alone (Baltieri & Suzuki, 2026).

For that reason, CSC's **irreversible vulnerability** is not proposed as a new name for precariousness or as the discovery that artificial agents can have mortality-like constraints. Artificial-life work has already treated death as a consequential transition: Korecki, Carissimo, and Lund (2023) use irreversible death states in reinforcement-learning agents and study how stories of those failures affect successor agents. More recently, Chen and Chen (2026) construct a mortality-grounded embodied agent and experimentally contrast persistent bodily consequences with body reset; when the body is reset, social effects no longer feed into the continued viability of the same life. These are direct neighbors to the present motivation.

The comparative gap addressed here is therefore more modest. CSC selects **four particular maintenance-and-consequence questions**, requires them to be indexed to a declared unit, boundary, timescale, and recovery regime, and treats the resulting profile as non-scalar and experimentally decomposable. The empirical contribution is correspondingly narrow: a matched terminate-versus-restore intervention is paired with a lives-gradient dose-response, payoff robustness checks, and a frozen-policy common-state probe. The novelty claim is not dimensionality, precariousness, death, or reset by themselves, but this particular operational package and the discrimination it supports.
...
Recent work makes the overlap clear. Korecki et al. (2023) use death states to create consequential experience whose traces can benefit successor agents. Chen and Chen (2026) go further by making mortality, accumulated bodily history, and persistent-versus-reset bodily consequences part of a continuing artificial agent. The present study should therefore be read as complementary rather than precedent-setting on artificial mortality: its distinctive empirical focus is the tightly matched recovery manipulation and the robustness package around the resulting policy separation.
...
Nor is this paper the first artificial-agent study in which mortality or persistent consequence matters. The closest recent neighbor, Chen and Chen's (2026) Synthetic Linguistic Agency preprint, experimentally manipulates whether bodily consequences persist or are reset inside a mortality-grounded linguistic agent; their target is the causal organization of embodiment, linguistic participation, and precariousness within one continuing artificial life. CSC asks a different comparison question: can four maintenance-and-consequence dimensions be indexed by a reusable unit/boundary/timescale/recovery declaration, and can one of those dimensions be isolated with a simple matched intervention whose result survives dose-response, payoff, and policy-level probes? This is a narrower novelty claim than either “mortality matters” or “life is multidimensional,” but it is also easier to test and falsify.
