...
where `A(sigma)` is assembly depth, `V_concern` is the spatial or social scope of relevant variables, and `tau_temporal` is the effective planning horizon. This decomposition is not offered as a final metric, but as a practical route for comparing chemical, embodied, and computational systems on a common scale. In particular, assembly depth provides one promising substrate-neutral anchor for the complexity term (Sharma et al., 2023).
...
This is what philosophy looks like when we have to build it. Instead of arguing verbally about whether a system is alive, we can vary one dimension at a time and ask which changes alter the persistence of life-like organization.
...
One open problem is calibration across substrates. The present paper offers a comparative grammar, not a finalized measurement protocol. In that spirit, Assembly Theory provides a useful external anchor for estimating the complexity term in `d_cog`, because assembly depth may allow chemical, embodied, and computational systems to be compared without collapsing them into a single material implementation (Sharma et al., 2023). Whether that proxy is robust enough for ALife benchmarking remains an empirical question.

The central lesson is simple: stronger life-likeness requires stronger forms of paid commitment. To build a more life-like system, it is not enough to increase complexity, realism, or behavioral flexibility. We must build systems whose organization matters because it must be maintained, carried forward, and can genuinely be lost.
