...
> `manuscript/08_references.md`, `REFERENCE_LEDGER.md`, `PAPER_CHARTER.md`, and the
> candidate bibliography have been synchronized. The assembled `MANUSCRIPT.md` is a
> mechanically generated mirror and must be regenerated from the current section files
> before any submission/export; any older self-citation string inside an unregenerated
> mirror is not an authority on publication status.
>
...
- **Self-citation live status — RESOLVED 2026-08-18.** Source-of-truth bibliography
  and ledgers now record CSC as submitted to *Adaptive Behavior* and Zhang (2026)
  as published in *Frontiers in Neuroscience* 20:1837760, DOI
  `10.3389/fnins.2026.1837760`. The older July wording is historical only.
- **Generated MANUSCRIPT regeneration — required before export, not a scientific
  blocker.** `manuscript/MANUSCRIPT.md` is assembled mechanically. Re-run
  `assemble_manuscript.py` after this status sync so its reference block exactly
  mirrors `manuscript/08_references.md`; until then, `08_references.md` and
  `REFERENCE_LEDGER.md` govern bibliographic status.
