...
**Figure 2** *(placeholder — to be drawn).* Experimental design and lifecycle regimes. Two independent REINFORCE agents share an identical reward function; the three regimes (real-stake, resettable, simulated-stake) differ only in what happens when an agent's energy reaches zero — permanent removal for the rest of the run, cheap reset to full energy, or reset accompanied by represented-danger tokens.
