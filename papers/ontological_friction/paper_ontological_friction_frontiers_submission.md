...
where $u(t)$ is control input and $\dot{x}_0$ is the no-control (baseline drift) trajectory. Executive friction is instantiated as integrated control effort:
...
1. **Within-behavioral CFA**: fit a correlated three-factor Miyake-style model (shifting, inhibition, flexibility) and test whether a second-order general factor provides acceptable fit ($\text{CFI} > 0.90$, $\text{RMSEA} < 0.08$).
...
**Prediction.** In a three-group comparison (OCD, $n = 30$; MDD, $n = 30$; healthy controls, $n = 30$), the OCD group shows meaningfully higher $\mu_{\text{sem}}$ than both MDD and control groups, driven specifically by elevated frequency of constraint modals.
...
**Falsification criterion.** If OCD $\mu_{\text{sem}}$ does not reliably exceed healthy control $\mu_{\text{sem}}$, the disorder-specific linguistic signature model is rejected.
