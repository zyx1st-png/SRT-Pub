...
### Def-ONT-1f: Vagueness Hysteresis Test for d-Value

d-value 是选择算子 $\hat{G}_\theta$ 的注意力边界——非固定存在的硬边界，而是主体基于最小自由能原则、具身环境与风险评估动态生成的选择范围。
...
- 若系统为无状态的软概率插值（如 softmax），扫描方向不影响输出，则：
\[
\Delta\tau_{hys}\approx 0
\]
- 若 $\hat{G}_\theta$ 基于最小自由能动态更新注意力边界，正反向扫描积累不同的具身-风险状态，路径依赖显现，则：
\[
\Delta\tau_{hys}>0
\]

$\Delta\tau_{hys}>0$ 表明 d-value 具有梯度更新特征（类比神经网络反向传播）：选择范围随扫描路径而动态调整，是 $\hat{G}_\theta$ 在具身环境中持续最小化自由能的可观测指纹，而非边界维持代价的体现。
