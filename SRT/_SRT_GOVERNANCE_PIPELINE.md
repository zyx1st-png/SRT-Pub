...
> **变更（2026-03-02）**：在每周文档质量审查后，新增"理论方向评审"环节，产出写入 `_SRT_WEEKLY_THEORY_REVIEW.md`。
...
4. 去冗余：重复段落 / 重复边界声明 / 重复标题清理
5. 更新 `_SRT_QUALITY_SCORECARD.md` 与 release 注记
...
## Part B：理论方向评审（新增，每周固定，追加到 `_SRT_WEEKLY_THEORY_REVIEW.md`）
...
### 2. 未解 Gap 状态
（_SRT_REVIEW_QUEUE.md 中 High 优先级条目进展）
| Gap | 优先级 | 本周状态变化 |
|-----|--------|------------|
...
| 评审项 | 数据来源 |
|--------|---------|
| 当前理论前沿 | `_SRT_SIGNAL_LOG.md`（本周记录） |
| 未解 Gap | `_SRT_REVIEW_QUEUE.md`（Pending 区） |
| 张力监控 | `_SRT_MATERIAL_LOG.md` + `_SRT_DAILY_REVIEW_LOG.md` |
...
- `_SRT_QUALITY_SCORECARD.md`（更新）
- `_SRT_WEEKLY_THEORY_REVIEW.md`（追加本周区块）
