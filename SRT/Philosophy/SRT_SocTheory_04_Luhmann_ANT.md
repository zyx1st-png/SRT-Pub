...
- **Mechanism**: $\hat{G}_\theta$ 在社会系统中执行传播选择，系统闭包 ($\hat{G}_{social}[L_1]=L_1$) 实现自创生。$\Psi_f$ 升高时 $\theta$ 发生防御性收缩（保守化），过高的模仿耦合 ($\kappa_{mimetic}>\kappa_c$) 触发替罪羊相变以释放拓扑拥挤。$d$-value 在转译过程中决定参数对齐的范围与成本——对齐失败时产生语义摩擦。
