...
$$
其中：
- **$\eta(L_2)$**：系统的迟滞系数（Hysteresis，→ `SRT_Social_MacroDynamics.md` §6.5），度量制度/文化网络抵抗拓扑更新的硬度；
- **$P_{adapt}^{(i)}$**：个体的可塑性带宽（Plasticity Bandwidth），近似定义为 $P_{adapt}^{(i)} \equiv d_i \cdot \gamma_i$（个体的关切维度上限 $d_i$ 与其参数更新率 $\gamma_i$ 的乘积）。
