...
**关联内容**：
  - 文件：`SRT/_SRT_EQ_HYP_MAP.md#G1-G2-G3`；`SRT/Operations/_SRT_REVIEW_QUEUE.md`
...
## [2026-03-12] 选题 #1 — 大众路线

**候选标题**：为什么同一件事，有人会愤怒，有人却只感到羞耻或关怀？
**切入角度**：从同一公共事件引发完全不同情绪反应切入，解释文化镜片如何参与“看见什么、感到什么、什么算理所当然”。
**选题原因**（时效性/争议点/SRT独特视角）：
- 时效性：Boas 情感镜片材料刚完成 A 级融入，适合做一条低门槛新入口
- 争议点：公众常把情绪差异归因为“性格问题”，忽略文化协议的门控作用
- SRT视角：用 `L_2^{culture}` 下沉到 `L_1^{affect}` 的机制，把“情绪分歧”重写成可解释的选择结构
**关联内容**：
  - 文件：`SRT/Philosophy/SRT_Social_Cognition.md`
  - 方程：Def-Soc-CLC-1；T-Soc-CLC-1；T-Soc-CLC-2
  - 实验钩（若有）：共享注意强度 × 情绪命名差异的跨文化/群体任务对照
**推荐平台**：知乎 / 微信公众号 / 头条
**风险提示**：避免滑向“文化全决定论”或“集体情绪都不理性”的误读，需保留生理底盘与反思性公共关切边界。

---

## [2026-03-12] 选题 #2 — 精英路线

**候选标题**：What Actually Drives a Consciousness State Transition? An SRT dual-driver hypothesis
**切入角度**：从“单一全局连通度指标是否足够解释麻醉诱导/恢复”这个争议切入，提出 SRT 的双驱动状态转移模型。
**选题原因**（时效性/争议点/SRT独特视角）：
- 时效性：Frontiers 2026 意识状态转移论文刚完成 A 级审核并回写
- 争议点：意识丧失与恢复究竟是单一整合度下降，还是多路驱动重配
- SRT视角：把“原级感觉驱动下降 + 额叶未知输入上升”写成比单指标更强的可证伪门
**关联内容**：
  - 文件：`SRT/Neuroscience/SRT_Consciousness_Mechanisms.md`；`SRT/Core/SRT_Core_22_Equations.md`
  - 方程：Ax-CONSC-MECH-9；H-C10
  - 实验钩（若有）：麻醉诱导/恢复双驱动拟合 vs 单一连通度模型比较
**推荐平台**：Twitter/X / Substack / Medium
**风险提示**：必须注明这是研究假设与区分模型，不宣称已取代 GNW/IIT，也不宣称已完成因果证明。
