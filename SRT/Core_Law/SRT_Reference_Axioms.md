...
适应度收益 (Fitness) 优先于真理 (Truth)。幽灵算子 $\hat{G}_θ$ 不是为了揭示 $L_0$ 的真相而演化的，而是为了最大化生存适应度。

$$\hat{G}_θ[σ] = \arg\max_{σ' ∈ L_0} P(\text{Fitness} | σ', θ)$$

**推论 A7-C1 (界面理论)**: 我们感知的时空是一个"用户界面"，旨在隐藏 $L_0$ 的复杂性以降低计算成本。$L_1 ≠ L_0$ 的映射，而是适应性压缩。

**推论 A7-C2 (奥卡姆从属)**: 算法简洁性是适应度的启发式近似，而非独立判据。
...
| 编号 | 名称 | 内容 | 源公理 |
|:-----|:-----|:-----|:-------|
| **T-Scale** | 尺度一致性 | $π_λ ∘ \hat{G}_θ ≈ \hat{G}_{θ,λ} ∘ π_λ$ | A5 |
