...
**潜在**：不是虚无，也不是任意幻想，也不是完全中性的平坦空间；而是被选择历史持续雕刻的可能性景观。它的倾向性结构不是预置的，而是选择过程的累积效应刻在可能性场上的拓扑。选择雕刻潜在域，潜在域的地形约束选择——两者共同涌现。
...
11. **潜在域预置论**：潜在域的倾向性结构不是预先存在的背景，而是选择过程累积雕刻的拓扑。ε 不是预置在 L₀ 中的先验目标，而是选择动力学的结构性不对称。
