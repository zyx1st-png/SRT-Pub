...
If one wants a compact bookkeeping vocabulary, borrowed from SRT but not required for the main argument, it is helpful to name three layers. `L0` denotes the open possibility space available to a system, `L1` its currently maintained manifest state, and `L2` the relatively stable structures sedimented by prior successful selection. These labels are optional: the substantive point is simply to distinguish possibility space, current organization, and retained constraint.
...
Consider a PettingZoo-style two-agent cooperative foraging environment. Each agent observes local resource, partner, obstacle, and threat variables plus a recurrent hidden state; cooperative harvesting yields more reward than solitary harvesting. In the compact bookkeeping map, the designed gridworld and its action-contingent futures supply the local possibility structure (`L0`), the currently occupied observation-action regime forms the maintained state (`L1`), and the trained policy plus recurrent hidden state provide the simplest form of sedimented structure (`L2`). Here `d` can be estimated from the effective rank of the Jacobian of action logits with respect to observation features across held-out trajectories. As a toy calculation, a normalized singular-value spectrum such as `[1.0, 0.95, 0.9, 0.8, 0.75, 0.7, 0.62, 0.55, 0.48, 0.42, 0.36, 0.3, 0.24, 0.18]` yields `D_eff ≈ 11.6`, which is the sense in which the agent is placed at `d ~ 10-20`. `eta` can be operationalized by measuring how strongly hidden-state perturbations continue to influence later actions; slower decay implies higher hysteresis. `Psi_f` is low-to-moderate but externally buffered, because training and inference consume real energy while depletion does not threaten persistence of the token agent. `V` is weak because failed regulation triggers reset rather than irreversible loss; a simple proxy would be `V_proxy = 1 - P(restore | failure)`. The same environment can also be used for the matched-agent comparison in Section 5, which makes the framework's discriminations more explicit. This worked example is more explicit than the other rows in Table 1 and should be read as a measurement recipe rather than as pilot data already in hand:
