...
This paper proposes such a criterion. Drawing on Selective Reality Theory (SRT), used here in a self-contained form as the author's own framework, it argues that the minimal signature of life-likeness is **costly selective closure**. A life-like system is not merely one that persists, computes, or reproduces. It is one that:
...
**Maintenance cost (`Psi_f`).** A system may display organized behavior while paying little of the cost itself. To count as strongly life-like, its organization must require ongoing expenditure. `Psi_f` denotes the cost of maintaining the selected state against drift, dispersion, or collapse. In biological settings this includes metabolic work; in artificial settings it may include energy, irreversible weight change, structural degradation, or other non-trivial maintenance burdens. Crucially, `Psi_f` is not gross energy expenditure measured by an outside observer. It is the maintenance burden borne by the system as a condition of preserving its own token organization. A large language model may consume substantial external GPU energy during training or inference while still ranking low on `Psi_f` in the present sense if failure does not threaten the persistence of its own state and if checkpoints make restoration cheap.
...
To make the framework more explicit, consider a PettingZoo-style two-agent cooperative foraging environment. Each agent observes its own energy level, local food density, nearest-partner distance, partner energy bin, obstacle flag, threat flag, previous action, and a small recurrent hidden state. Cooperative harvesting yields more reward than solitary harvesting but requires short-term sacrifice and coordination.

In such a setting, `d` can be estimated from the effective rank of the Jacobian of action logits with respect to observation features across held-out trajectories. If that rank clusters, for example, around 12-18 across runs, the system would plausibly be assigned `d ~ 10-20`. `eta` can be operationalized by measuring how strongly prior hidden-state perturbations continue to influence present actions over several timesteps; slower decay implies higher hysteresis. `Psi_f` is low-to-moderate but externally buffered: training and inference consume real energy, yet depletion does not threaten the persistence of the agent's token organization because the policy can be restored from stored weights. `V` is weak because failed regulation triggers reset rather than irreversible loss.

The point of this example is not to claim a completed measurement pipeline. It is to show how the four dimensions can be tied to an explicit system description instead of remaining purely verbal. The same logic can then be extended to other rows in Table 1.
...
To isolate vulnerability from changing ecological pressure, reward functions, observation spaces, and background resource density should remain matched across conditions. Population size can be held constant through fixed pairings or replacement slots that preserve resource density, so that any observed difference is attributable to lifecycle regime rather than an accidental shift in available resources.
...
The proposal is compatible with threshold-based approaches to minimal cognition, including Unlimited Associative Learning, while remaining focused on life-likeness rather than consciousness per se (Birch et al., 2020). Cross-substrate calibration remains open; assembly depth may provide one useful anchor for the complexity term in `d_cog` (Sharma et al., 2023). A longer-term question is whether stronger forms of active closure also help explain increasingly integrated individuality and evolutionary transitions in artificial systems (Watson and Szathmáry, 2016).
