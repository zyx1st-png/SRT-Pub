...
## §2 标记并写入 `_SRT_REVIEW_QUEUE.md` 的范围
...
3. 若距上次 ≥ 22h 或手动触发：
   ...
   b. 运行 §2 检查项 → 收集待审列表
   c. 执行 §1 修复（直接编辑文件）
   d. 追加 §2 待审项到 _SRT_REVIEW_QUEUE.md
   e. 追加本次运行摘要到 _SRT_DAILY_REVIEW_LOG.md
   f. git commit（若有修复）
...
2. `_SRT_REVIEW_QUEUE.md` 中的标记是建议性的，人工处理时可以判断为误报并标注 `[FALSE_POSITIVE]`。
