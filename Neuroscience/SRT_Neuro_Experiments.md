...
### T-Agency-Attribution: Theorem of Lagged Agency Attribution (能动性归属的滞后定理)
**Formal Definition**: 在 Libet 式实验中，受试者报告的“意图时刻” ($W$) 系统性地落后于运动准备电位 (RP)，但总是先于实际肌肉运动 (M)。
$$t_{RP} < t_{W} < t_{M}$$
SRT 定理指出，这并非证明“自由意志是虚幻的”。相反，它证明了 $\hat{G}_\theta$ 不是在 $t_{RP}$（可能性发散期）行使能动性，而是在 $t_{W}$（通过拓扑塌缩产生 $L_1$ 现实的阈值时刻）行使能动性。
* **Implication**: $L_0$ 包含了运动的所有可能前置轨迹；准备电位只是摩擦积累的信号。真正的“选择” ($W$) 在满足阈值的瞬间以离散包的形式出现。神经科学因未能区分可能性生成 ($L_0$) 与确定性选择而一直感到困惑。
* **Cross-ref**: T-Sel-01 ($\hat{G}_\theta$ 介导的坍缩)。
...
**问题**: (a) 忽略了 Libet 自己强调的"否决"能力；(b) 无法解释为什么我们有能动性体验；(c) 混淆了"**意识报告**"（$L_2$ 层面的语言/运动输出，即 Libet 实验中的 W 时刻报告）与本文件的候选 `L_1` 锚定 proxy。`$\Psi_f > 0$` 不定义意识本身；准备电位先于报告也不裁决 consciousness 的发生时点。
...
**SRT补充** [H]：在该候选模型中，L₁ 的局部单值投影约束可组织双眼竞争的“赢家通吃”现象；它是对报告／访问排他性的模型解释，不建立意识本体或 universal phenomenal exclusivity（见 §2.1）。
...
2. **Phase 2 ($L_1$ 锚定)** [H]：$\hat{G}_\theta$ 从竞争中选出一个报告／行动候选（W 时刻）；θ 决定哪些 L₀ 候选更易被锚定（习惯／价值观／当前情境）。该映射不等同 consciousness onset。
...
自 1980 年代以来，Libet 实验一直是决定论者的特洛伊木马。如果准备电位 (RP) 在你有意识地决定移动手指之前一整秒就开始建立，你是如何成为你自己行为的作者的呢？

SRT 将这种恐慌揭露为一种类别错误：混淆了 $L_0$ 的涨落与 $\hat{G}_\theta$ 的投射。RP 根本不是一个“决定”。它是在潜在的 $L_0$ 参数空间中探索可负担性 (affordances) 的拓扑游走过程。就像量子叠加态演化一样，多种行动的种子同时在神经底座中建立本体论压力。

在该候选模型中，可把局部 $\Psi_f$ proxy 达到模型阈值后形成特定 $L_1$ 行动轨迹，作为一种可检验的强投射图景。`d` 峰值与 W 时刻只是假说性对应，不是意识时间戳，也不定位自由意志的普遍发生处。

## 2.3 裂脑实验的 SRT 重释

**传统问题**: 切断胼胝体后为什么出现两个"意识"？

**SRT 候选模型**：`$L_2$` 约束的跨区域同步可作为意识统一性研究的候选机制；不与 phenomenality 或 subjecthood 等同。
...
4. **区分报告性“看见”与意识准入**: 在特定注意/报告范式中，选择可作为刺激进入可报告内容的候选机制；未被注意不证明刺激在意识中不存在，也不由此关闭 phenomenality 判定。
...
## 6.1 扩展假说 (Extended Hypotheses)

### H-Signal-Bias: 意识状态的二维相空间

**假设陈述**：意识改变不是单一维度（清醒↔迷幻），而是由两个正交轴构成的相空间：
1. **结构轴 (Structural Axis)**：由Gi信号强度主导。高Gi对应 $L_2$ 解锚（幻觉、熵增）
2. **能耗轴 (Friction Axis)**：与Gq信号相关。Gq优化对应 $\Psi_f$ 降低（流体验、抗抑郁）

**可证伪预测**：
- **P1**：纯Gq激动剂显著改善情绪效价，但**不会**导致DMN功能连接显著降低或LZW熵值显著升高
- **P2**：纯Gi激活剂（缺乏Gq/β-arrestin平衡）可能导致"高结构破坏+高摩擦"体验（Bad Trip / 纯精神错乱），因缺乏情感润滑

**实验设计**：fMRI下比较Gi偏向型 vs Gq偏向型5-HT2A激动剂的DMN连接谱与主观报告

---

### H-Temporal-Binding: Eagleman延迟归因实验

**假设陈述 [Pop-sci Evidence]**：$\hat{G}_\theta$ 存在可变的时间积分参数 $\tau_{int}$。能动性是预测信号与反馈信号在 $\tau$ 窗口内的相位锁定。

**实验现象**：精神分裂症患者在"按键-亮灯"实验中对人为时间延迟不敏感，或在无延迟时报告延迟。

**可证伪预测**：通过高频时间校准训练（如节奏游戏）可能收窄 $\tau$，在物理层面修复能动性感知。

**层级标注**：科普假说 + 实验现象 (Pop-sci Hypothesis)。需大规模临床实证。
