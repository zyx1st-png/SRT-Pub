...
此时，$\hat{G}_\theta$ 是在旁观的——或者更准确地说，是在等待。只有当局部的 $\Psi_f$（摩擦力）达到临界特征值时，算子才必须进行强投射，将广泛的可能性波函数切断，坍缩为一个特定的 $L_1$ 轨迹（“现在移动！”）。这个坍缩事件由于需要耗费极大的带宽（$d$ 值的瞬时峰值）而表现为意识时间戳 ($W$)。自由意志在于决定在相空间何处允许发生坍缩，而不是在生成早期在 $L_0$ 中沸腾的无特征噪声。
