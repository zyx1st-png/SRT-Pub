...
The detailed comparisons formerly in §1, §2.2, §3, and §4 have been moved to [`Neuroscience_Annex/10_Integration_Theory_Comparisons.md`](../Neuroscience_Annex/10_Integration_Theory_Comparisons.md).

These materials are bridge/interface comparisons between external consciousness theories and SRT vocabulary. They do not define SRT Core primitives.

Def-Phi-Unity, Ax-CLIN-1b, Ax-CLIN-2/3/4/5/6, T-INTEG-1, and C-INTEG-1 remain the formal owner-file anchors.

IIT `Phi` mappings do not redefine Def-Phi-Unity or C-INTEG-1. FEP/free-energy mappings do not redefine `Psi_f`. GNWT ignition does not define the `L_0 -> L_1` transition. HOT self-reference does not define `\hat{G}_\theta`. Orch-OR remains a hypothesis-level physical-mechanism comparison only.
