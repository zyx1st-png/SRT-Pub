...
**SRT 立场**（对计算功能主义的修正）：
- **改进 (a)**：在一个声明的 stake-coupling 模型中，中文房间操作者可被赋予较低 $d(\theta)^{proxy}$ 与 non-binding `Ψ_f` 读数；这只削弱该模型内的 consciousness 支持，不以 Cor-CONSC-1 三项合取证明无意识。纯符号操作本身也不足以推出意识。
- **改进 (b)** [P3/P4 候选]：$\nabla\Psi_f$ 的方向、幅度和动力学特征可作为感受性特质的候选物理 proxy。*边界说明*：二者的等同没有建立；当前只能间接测量，不能准入或排除 phenomenality。
- **多重可实现性接口** [P3/P4 候选]：不同基质可实现相同 $\hat{G}_\theta$ 功能；`Ψ_f` 热力学模式可作为比较候选，但模式匹配不证明感受性质相同，模式不匹配也不证明感受性缺席。

**理论一致性要求**：
- IC-Func-1（∇Ψ_f可测性要求）：若该候选模型要主张感受性特质与 ∇Ψ_f 存在稳定对应，必须提供操作化测量方法；当前不得使用等同号
...
- FC-Func1-2：若麻醉相关 `Ψ_f` proxy 变化稳定先于独立意识相关证据变化，则支持该候选桥；反向或无区分结果会削弱它。本检验不预设 `Ψ_f>0` 是意识必要条件。
