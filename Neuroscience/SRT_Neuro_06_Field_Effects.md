...
* **Implication（P3/P4 候选）**：低频 LFP 产生的全局电场可作为复合算子拓扑完整性的候选物理机制。关闭场耦合若导致报告／绑定碎片化，会支持该模型；场耦合不是统一体验者或 consciousness 的必要条件。
...
$\mathcal{C}_{field}$（注意力场同步算子）：在声明具体任务、网络范围、读出和比较函数的模型内，使各局部选择算子相对同一全脑协调目标形成可检验的方向一致性。这里的“全脑”是模型边界，不是 universe-wide optimum；同步本身也不证明目标正确。

**d-value 与有界可达比较的连接**：场耦合若确实扩大 stake-coupled 的可探索路径，可能使 $\hat{G}_{macro}$ 暴露单个 $\hat{G}_{micro}$ 未纳入的任务内替代；路径数量增加本身不等于 canonical `d` 增长，也不提高抵达某个全局最小配置的概率，除非目标函数、可达域、时域和读出已声明。场耦合失败（如解离或麻醉加深）可作为整合与任务内协调下降的候选机制，不证明系统偏离了宇宙级最优。
...
**候选模型定义**：在不预设 phenomenality admission 的前提下，设 $\mathcal{R}_\theta$ 为声明模型中的 raw phenomenal-stream representation；$\Pi_{L_2}$ 为将候选 $L_1$ 内容投射至 $L_2$ 符号／概念层的有损压缩算子。该构造不建立主体、第一人称体验或感质存在。则：
...
详细外部理论比较（突触同步假说、全局工作空间理论、整合信息理论）已迁移至 [`Neuroscience_Annex/07_Field_Effects_Interface.md`](../Neuroscience_Annex/07_Field_Effects_Interface.md)。

这些材料是 bridge/interface 比较内容，不定义 SRT Core primitives。GWT ignition 不定义 SRT 场绑定机制。IIT `Φ` 不定义 SRT Resonome、QUALIA-1/2 或场绑定阈值。突触同步材料作为候选神经代理，不取代 Ax-FIELD-1、Def-Ephaptic-Binding、T-FIELD-2。

**Owner formal anchors remain**: QUALIA-1 / QUALIA-2, Ax-FIELD-1, Def-Ephaptic-Binding, T-FIELD-1 / T-FIELD-2, C-FIELD-1, `κ_sync`, `Ĝ_macro`, and H-Field predictions.
...
1. **检验神经元中心主义**：电磁场可作为候选整合机制接受独立检验；不预设它是意识基质
    
2. **接受向下因果**：必须接受场可以因果性地影响神经元，而非仅仅是神经元的副产品——这挑战了还原主义的默认假设
    
3. **重新评估意识研究方法论**：若场效应提供独立增量，单神经元记录可能不足以解释部分整合现象，可发展场测量技术；这不声称捕捉意识本质
...
> **候选预测**：如果能够选择性地阻断互噬触（ephaptic coupling，细胞间电场耦合）而不影响突触传递，该模型预测绑定报告会出现可测变化；这检验场耦合的增量，不建立或排除意识体验。
...
4. **人工意识的场候选**：场是否为某类 $\hat{G}$ 模型的实现维度？其有无不得直接裁决纯数字 AI 的 consciousness。
...
## GRT Interface Annex Pointer

GRT / Hunt & Schooler (2019) 核心主张及 SRT-GRT 会聚点比较已迁移至 [`Neuroscience_Annex/07_Field_Effects_Interface.md`](../Neuroscience_Annex/07_Field_Effects_Interface.md)（§6.1 GRT核心主张，§6.2 SRT-GRT会聚点）。

GRT 共振比较是 bridge/interface 材料。GRT 共振不定义 `κ_sync`、`Ĝ_macro`、`\hat{G}_\theta` 或 T-FIELD-2。会聚表中的 `✓` 标记表示候选功能类比，不是本体论等同。

§6.3 SRT独特贡献保留在本 owner 文件，因为它包含 SRT 内部分化主张。
...
这解释了为什么致幻体验常被描述为"自我消融"、"万物一体"——这可作为 $d^{subjective/provisional}$ 上升 / self-boundary loosening 的现象学 proxy，而不是实际 `d -> infinity`。
...
1. **从神经元到场**：场拓扑是候选整合机制，不是 consciousness identity
2. **从还原到整合**：意识研究可同时检验全局场模式与局部神经元活动，不把任一方当作 universal admission
