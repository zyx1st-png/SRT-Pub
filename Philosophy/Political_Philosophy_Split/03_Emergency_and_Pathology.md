---
id: SRT-POLITICAL-PHILOSOPHY-SPLIT-03
type: summary
tags: [PoliticalPhilosophy, Split, Emergency, Sovereignty, Pathology, LethalL2]
status: active_v1
layer: L1
epistemic_layer: bridge
claim_mode: navigation
claim_level: P2-P4
canonical: false
---

# Emergency and Pathology

> 角色：`SRT_Political_Philosophy.md` 的导航性拆分页。  
> 不替代主文；用于局部阅读、引用与后续扩写。

## 1. Constitutive Floor Priority
\[
\text{Priority} \Rightarrow \text{protect constitutive floor before expandable scaffold}
\]

最短理解：
- 在嵌套冲突中，优先保护使闭合得以继续存在的地板
- 而不是只保护更高层扩张能力

## 2. Emergency Legitimacy
\[
\text{Emergency legitimacy} \iff \text{minimum necessary interruption preserving } C_{FBC}
\]

最短理解：
- 危机中的决断合法性不来自人格、意志或主权神学
- 它只来自：是否以最小必要方式保全更多构成性存在与未来分支

## 3. Exception Is Not Self-Justifying
最短理解：
- SRT 承认例外状态不可消除
- 但拒绝例外永久化、人格化与神圣化
- 一旦危机逻辑变成常态合法性，政治病理就形成了

## 4. Political Lethal L2
最短理解：
- 政治最危险的时刻，不是只有暴力最强的时候
- 而是制度越来越会替代真实选择、制造参与感、让主体失去重新进入现实塑造的能力

## 5. Naturalization and Relapse
最短理解：
- 意识形态最强时不是被高声宣告时，而是被看成“自然现实”时
- 革命最容易失败时不是旧秩序尚强，而是新秩序迅速复制旧闭合结构时

## Back-links
- 主文：[`../SRT_Political_Philosophy.md`](../SRT_Political_Philosophy.md)
- 伦理主轴：[`../SRT_Philosophy_Ethics.md`](../SRT_Philosophy_Ethics.md)
- `T_dir` canonical：[`../../_SRT_T_DIR_CANONICAL.md`](../../_SRT_T_DIR_CANONICAL.md)
