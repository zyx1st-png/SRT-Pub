...
Registered translation candidates: `D05-T01`–`D05-T05`. One (`D05-T02`) is the best preservation case and is presented at full strength. One (`D05-T03`) is ruled inadmissible for this obligation on type grounds and is carried to D09.
...
Three inherited limits constrain every verdict below and are the direct reason the final verdict is *qualified* rather than *supported*.
...
**(b) Granularity of `M_t`.** Frozen §3.3 types `M_t` as the set of methods implementable for the problem and §5.2 as `𝔘_t↾_{Q_φ}`. Neither fixes how a method is individuated. The verdict is granularity-sensitive: see `D05-T02`. This audit uses an individuation fine enough that the criterion remains calibratable, on the ground that frozen §7.5–7.7 and §19(6) require the criterion to discriminate result innovation, local technical innovation and forcing against two controls. A granularity under which every relative-consistency proof counts as the same method would make Control A indistinguishable from forcing and is therefore excluded by the frozen calibration architecture itself, not by a preference of this audit.
...
| strongest failure argument | two grounds. First, C5-op is a condition on `(b,m) ⤳ o`, and the Cohen relation's load-bearing intermediate output is the extension `N`, not the consistency statement; D03 Cohen §4.2 separates the semantic construction route from the finite-fragment reduction as two routes with different premises and outputs, so collapsing them discards a distinction the primary record itself draws. Second, the granularity that makes this candidate work also makes the criterion inert: under it, any two relative-consistency proofs preserve C5-op, so Control A could not be distinguished from forcing, contrary to frozen §7.5–7.7 |
| provisional verdict | **preserves only under a granularity independently excluded by the frozen calibration requirement**; does not succeed as an admissible reconstruction |
...
This is a serious case and the audit does not dismiss it. Two things defeat it, neither of which is a preference of this dossier.

**First, it discards a distinction the primary record draws.** D03 Cohen §4.2 separates the event-stage semantic construction from the finite-fragment reduction as two routes with different premises and different outputs, and the Cohen dossier's claim-type matrix keeps `N ⊨ ZF+AC+not-CH` and `Con(ZFC) → Con(ZFC+not-CH)` as distinct claims. `D05-T02` preserves C5-op only by taking `o_C` to be the second and dropping the first. But the extension `N` is not an optional intermediate: it is the object Part II verifies the axioms in, the object Lemma 20 establishes `not-CH` in, and the object the truth lemma is about [`COHEN1963-CH-I`, p. 1147; `COHEN1964-CH-II`, pp. 105–109]. A translation that reproduces the consistency statement while having no image for `N` has not reproduced the relation `(b,m) ⤳ o`; it has reproduced a consequence of it.

**Second, the granularity that makes it work disables the frozen criterion.** If `M_t` is individuated by proof-goal type, then any two relative-consistency proofs preserve C5-op with respect to each other. Frozen §7.5–7.7 requires the criterion to discriminate a result innovation (Control A), a local technical innovation (Control B) and forcing; §19(6) makes triple calibration a submission condition. Under the coarse individuation, Control A — an important result inside a mature regime — and forcing would fall on the same side of C5-op, and the calibration required by the frozen strategy could not be performed at all. The coarse reading is therefore excluded by the frozen architecture, not by this audit's judgement.

What survives from `D05-T02` is not preservation but a genuine boundary on the verdict, recorded in §9 and §14(b): the finding is stated relative to a method individuation that the frozen strategy constrains only indirectly.
...
| `D05-T02` relative-consistency proof type | yes | preserves only under an individuation the frozen calibration architecture excludes; and has no image for `N` | D03 Cohen §4.2 two-route separation; `COHEN1964-CH-II`, pp. 105–110; frozen §7.5–7.7, §19(6) |
...
- `COHEN1964-CH-II`, p. 110 is the strongest record *against* the verdict: it is the basis of `D05-T02`. It is not set aside on evidential grounds but on the two grounds in §7.
...
3. **The verdict is granularity-relative.** §4.1(b) fixes the individuation by appeal to the frozen calibration requirement, which is a sound but indirect constraint. A future strategy revision that fixes `M_t` granularity explicitly could move this verdict in either direction.

### 9.4 Why not *unresolved*

Because the five registered candidates include the two that the merged dossiers make available in their strongest form (`D05-T02` on the operation-type side, `D05-T04` on the continuity side), both were steelmanned before assessment, and each fails on a documented record rather than on absence of evidence. A bounded result is available and is reported.

### 9.5 Why not *defeated*

No registered candidate succeeded. `D05-T02` came closest and is the reason the verdict is qualified rather than supported; but it survives only by dropping `N` from the output and by adopting a granularity that would disable the frozen criterion's own calibration.
...
| `EVD-D05-0004` | Verified technical determination of whether the documented Gödel-stage operation is *provably incapable* of generating a model of `not-CH` from a ground model satisfying `V=L` — that is, whether the relevant minimality/absoluteness facts about the constructible class turn the observed non-reproduction into a demonstrable impossibility. | If verified, this converts "no documented Gödel-stage operation does it" into "the documented operation cannot do it," which would remove the largest part of the §9.3 gap and could upgrade `EVD-D05-0001` to supported. **Not asserted here**: no source in the merged dossiers establishes it, and this audit added none. |
...
| That shared proof type establishes C5-op preservation. | `EVD-D05-0001` | §7 assessment | defeated | Two-route separation and calibration requirement block it. |
...
4. Is the documented Gödel-stage operation *provably* incapable of yielding a `not-CH` model from a `V=L` ground model? (`EVD-D05-0004`.)
5. Which method individuation does the frozen strategy actually intend for `M_t`, and does the answer disturb §9? (§14(b).)
...
**(b) `M_t` granularity is not fixed.** Frozen §3.3 and §5.2 type `M_t` without individuating "method." `D05-T02` shows the C5-op verdict is granularity-sensitive in a way that matters. The frozen calibration architecture (§7.5–7.7, §19(6)) constrains granularity indirectly and sufficed here, but the constraint is inferred rather than stated.
