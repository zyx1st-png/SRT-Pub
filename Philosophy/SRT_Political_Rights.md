...
4. **抗捕获性假说**：d-mimicry 是否会在重复观测中留下可识别的不一致，是待检验的模型假说；“真实 d 倾向最终浮现”不作保证，需要独立操作化、对抗性测试与失效条件
...
```
代理者过去一段时间的决策后果
→ 其经验后果、负担与成本代理可能被被代理群体感知
→ 投票聚合部分主观信号
→ 产生关于代理方向的低精度候选证据
→ 由另行声明的委托框架决定是否及如何复核授权
```

选民不需要理解 d 值；他们对经验后果的反应可被模型当作候选信号。该信号与 canonical `Psi_f` 的映射保持 OPEN，也不证明代理者真正整合了何种关切。

**候选信号条件与潜在失效模式**：

| 条件 | 投票效果 |
|------|---------|
| 经验感受与可观测后果代理较一致 | 预测信号保真度可能较高；仍需验证 |
| 决策后果在选举周期内可见 | 预测信号时效性可能较高；仍需验证 |
| 大样本、重复进行 | 可能降低部分噪声；抗捕获性需对抗性测试 |
| 经验感受被宣传或信息不对称系统性扭曲 | 潜在信号失真 |
| 后果时间尺度超过选举周期 | 潜在信号滞后，d-mimicry 可能持续 |
| 决策作用域超出选民评估能力 | 潜在信号精度下降 |

**模型特定的失效假说**：若主观经验信号与可观测后果、负担和成本代理显著分离，投票可能只记录短期感受而难以提供可靠的 d 倾向证据。它不是所有民主失效的统一解释；这些代理与 canonical `Psi_f` 的映射保持 OPEN。

---

## §7 现有民主的历史／候选比较：低精度信号模型
...
```
金钱/选举作为筛选机制
→ 可能奖励能说服他人“我指向正确方向”的能力（d-mimicry）
→ 这种能力不证明 wider-scope consequence integration
→ 可能提高选择 d-mimicry 代理者的风险
→ `L_2` 层的可观测负担、成本或长期后果可能被短期感受遮蔽
→ 表面稳定与隐性负担增长的关系仍需经验检验
```

这是一个关于筛选机制的候选动力学假说，不是已验证的道德或制度结论。
...
| 验证信号 | 主观经验信号 | 可观测后果／负担代理 + 主观经验信号；与 canonical `Psi_f` 的映射 OPEN |
| 时间覆盖 | 选举周期（4-8年） | 系统性覆盖长时间尺度 |

**历史启发式的继承**：下列映射只记录候选先验与校正思路，不验证任何制度或主体的合法性：

$$\text{历史筛选信号（金钱/地位）} \to \text{候选低精度先验，需验证}$$
$$\text{d倾向代理评估} \to \text{候选校正信号，精度与规范效力均 OPEN}$$
...
**候选结构风险**：长时间尺度决策的信号滞后或失真

```
短期决策链：决策 → 可观测后果展开 → 进入主观经验信号 → 选举提供候选证据 → 委托框架复核
（时间尺度4-8年时闭环可能较易观测，仍需验证）

长期决策链：决策 → 可观测后果展开（数十年）→ 选举信号可能滞后或失真
（决策者可能已卸任；因果链可能模糊；d-mimicry 的代价是否显现需经验检验）
```
