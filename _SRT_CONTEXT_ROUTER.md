...
7. Before hardening a new repeated SRT / GRG term-of-art, check `_SRT_TERM_ROUTER.md` plus the current owner; exploratory conversation labels may remain working labels without admission.
...
`_SRT_TERM_ROUTER.md` is navigation only. It may tell you that two labels collide or point to the same owner, but it cannot settle the definition by itself.
