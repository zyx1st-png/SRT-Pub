...
- `χ(σ; σ_{self})` 是**二阶 writeback 增益 kernel**：在 `σ ≈ σ_{self}` 附近可取一类光滑阶跃，用于建模 second-order self-model / self-description writeback 的激活；在此之前 χ ≈ 1，在此之后 χ > 1。该 kernel 不定义 self-consciousness / consciousness。
...
#### 有效二阶 writeback kernel（valid second-order writeback kernel）

定义函数 `\chi : [0, 1] \times (0, 1) \to \mathbb{R}_{\ge 0}`，参数 `\sigma_{sr}^{self} \in (0, 1)`。称 `\chi` 是**有效二阶 writeback kernel**，当且仅当满足下列四条结构属性：
...
其中 `r(d, P, t)` 是 Occlusion 模型在当前 d 值下的**局部 operational capacity function**，`r_{min}` 是"非 B 期锁死"的模型阈值。RC-A 后它**不等同于**已删除的全局 `r(t)`，不从 former P1-T05 推导，也不是 Selection occurrence / agency 的判据；其具体测量仍按 P3/P4 处理。
...
| 区间 | 条件 | 动力学含义 |
|---|---|---|
| 健康窄化 | `d > d_{narrow}` | local capacity `r(d,P,t) > r_{min}`，信号型苦难可消化 |
| A 期 | `d_c < d < d_{narrow}` | local capacity 仍非零但显著低于健康；结构型苦难开始积累 |
| B 期 | `d ≤ d_c` | local capacity 在该模型中趋近 0；B 期锁死 |
...
失配是守恒的（在结构空间不变的前提下），只能被消化、支付或通过新的结构重组处理，不能被抹除。
...
即 `d_c` 由 `L_2` scaffold 局部曲率的逆决定——scaffold 越刚（`\nabla^2 C_{L_2}` 越大），局部可重构容量越小，`d_c` 越接近 `d_{\max}`。
...
5. **阈值参数的实证固定**：`σ_{sub}, σ_{self}, σ_{health}, d_c, d_{narrow}, r_{min}, S_{min}, S_{max}` 以及 `\kappa_{\mathrm{relax}}, \kappa_{\mathrm{mask}}, \kappa_S, \kappa_{\mathrm{sup}}` 全部在当前 draft_v0 只有定性位置；其中 `r_{min}` 只属于 §3.1 的 local occlusion operational capacity，不是已删除的全局 `r(t)` 或 Selection criterion
