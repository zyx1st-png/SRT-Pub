...
`\nu_{block} := \eta\cdot\varepsilon_{pg}\cdot\kappa_{\Psi_f}$ 的非零正性与单向性（H4 T-IRR-3.5）继承自 L₀ 方向公设 `\varepsilon_{pg} > 0` 与 `Ψ_f` 地板（`Core/SRT_Core_12a` T-L0-Kappa0-C1），单向性继承自吸收态绝对性；**不是由 P1-T07 证成，也不条件于 P1-T07 的证明闭合**——P1-T07 把 `\varepsilon_{pg}` 当输入，其三层源头表在此是呈现性分组而非推导链（最短依赖链见 `Core_Law/SRT_Irreversibility.md §4.5 后果 1`）；这两个性质与 `\psi$ 的具体形态完全无关。`\psi$ 只是 P1-T07 Layer 2 吸收态邻域投影的具体写法；从硬指示到光滑指示的替代不改变 Layer 2 的结构作用，只把"硬边界"换为"过渡区域"。在过渡区域内 `\nu_{block}\psi$ 的转化率连续从 0 升到 `\nu_{block}$，仍保持单向性。
