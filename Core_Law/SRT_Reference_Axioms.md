...
不同尺度的算子可在选择—约束—代价—历史写回这套共同语法下作条件性比较；物理选择到生物选择的谱系连续性是逐尺度 P2/P3 主张，不要求、也不定义一个位置无关的“原初全局算子”。bare $\Omega$ 依 `_SRT_SYMBOL_TABLE.md` 只表示 Ontological Consistency。

任何跨算子比较都必须在正文声明所比较的结构、尺度映射、约束、时域和失败条件；A12 不为此注册新的跨仓符号。
...
- 深层连续性（A12）把物理选择（$d \approx 0$）到生物选择（$d > 0$）保留为逐尺度结构相容候选；它不从一个原初全局算子推导连续谱系，也不把 bare $\Omega$ 用作该对象。
