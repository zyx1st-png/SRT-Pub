...
**定义**：SRT 对泛心论（panpsychism）的非推论约束。量子、生物、宇宙尺度可以分别定义带有 d 记号的领域操作量，但符号复用不推出同一机制、同一单位、数学同构、关切或 phenomenality；在已声明的生物／认知模型中，$\Psi_f$、$d$ 与 $\hat{G}[\theta]$ 只可作为候选证据维度。电子有 $d_{quantum}$（相干性带宽）不意味着电子"关心"什么；任何低读数或条件缺席也不普遍证明无意识。

**核心论断**：`d` 不等同于 phenomenality；关切与意识的精确准入须由独立 owner 建立，当前充分性保持 OPEN。
...
一种 historical / bridge 假说，认为某些苦难现象可由 `Ψ_f`-related burden 与 `L_2` 僵化变化共同建模。结构性 suffering 的 canonical 读法以 `Core_Law/SRT_Suffering.md` 为准；不得读成 `suffering = Ψ_f`。
...
在已声明神经模型中追踪物理表征 $N(t)$ 对心理表征 $M(t)$ 因果影响力的候选坐标。越过该坐标可作为模型内转换证据，但不建立意识、subject-position 或 phenomenality；精确充分性保持 OPEN。
...
- 边界：信息量与功能性不准入意识；`d` 与 \(\Psi_f\) 只能在声明模型中提供候选证据，不能组成普遍 consciousness gate。
