...
bounded scope 下，承担节奏的重复和转场保留；纯空句按 SKILL.md 的已有授权处理，未授权删除项才列待确认清单。
...
下面是合成案例。任何具体化都必须来自输入；缺信息时保留概括程度并在正文外说明缺口。

### A. `status`

原文：

> 本次优化显著提升了系统整体性能，并有效改善了用户体验。首页接口从 800ms 降到 120ms，之前那批卡顿反馈也少了很多。
...
> 该能力不是一个简单的配置选项，而是一套面向未来的系统性机制。它会一起改缓存策略、重试逻辑和超时设置。
...
> 真正的竞争力不是功能堆砌，而是体验细节。功能补得再快，如果延迟高、引导乱、错误提示看不懂，用户还是不会留下来。
...
如果删完只剩空架子，只能重组输入已有事实；缺材料时说明缺口，不补造事实句。
