...
- **Accepted / continued machine-analysis retrieval rule.** When the author explicitly accepts, approves, adopts, or instructs preservation/continuation of a machine-generated analysis or analysis package, the non-superseded analysis has **required retrieval value for continuation by default**, even when its epistemic authority remains `M`, `canonical: false`, `parked`, `downstream`, support, or companion material. A bare continuation signal such as `继续` / “continue” immediately after one coherent analysis path is treated as **package-level directional acceptance for retrieval** unless the same turn corrects/rejects part of it, the prior turn contains unresolved mutually exclusive branches, the author explicitly says the continuation is exploratory-only, or a specific item is already under an explicit pending-confirmation gate. This affects retrieval continuity only; it does not upgrade each machine sentence to A1. Author-source record: `01_Source_Intuition/SRT_AUTHOR_ADJUDICATION_CONTINUE_DIRECTIONAL_ACCEPTANCE_2026-09-24.md`. Only an explicit later narrowing such as `EXPLORATORY`, `LOCAL_ONLY`, `SUPERSEDED`, `REJECTED`, `RETIRED`, or a later controlling adjudication may demote current retrieval priority. Preserve earlier material as provenance when later semantics supersede it.
- **New term / working-label rule.** Dialogue may freely introduce temporary working labels, but before a new repo-wide SRT/GRG term-of-art is hardened, search `_SRT_TERM_ROUTER.md` plus the relevant current owner(s). Classify the proposed wording as an alias/same burden, overloaded same-name use, partial overlap, genuinely distinct burden, or explicit working label. Prefer an existing term when it already carries the burden; if a new term is retained, record its owner, distinction, aliases and failure/merge condition. Do not use the router itself as definition authority.
