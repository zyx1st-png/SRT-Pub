...
- **Accepted / continued machine-analysis retrieval rule.** Author-owned semantics and exceptions are in `01_Source_Intuition/SRT_AUTHOR_ADJUDICATION_CONTINUE_DIRECTIONAL_ACCEPTANCE_2026-09-24.md`. Operationally: accepted / boundedly continued non-superseded M analysis must remain routeable and be consulted when the relevant topic enters scope; it is not a universal fresh-session preload and it does not become A1 merely because the author continued.
