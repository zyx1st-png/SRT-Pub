...
- **Accepted machine-analysis continuity rule.** When the author explicitly accepts, approves, adopts, or instructs preservation/continuation of a machine-generated analysis or analysis package, the non-superseded analysis is **load-bearing continuation context by default**, even when its epistemic authority remains `M`, `canonical: false`, `parked`, `downstream`, support, or companion material. Those labels limit authority / execution scope; they do not by themselves lower continuation importance. `continuity_role` is an optional explicit override / retrieval hint, not a prerequisite for being load-bearing. Only an explicit later narrowing such as `EXPLORATORY`, `LOCAL_ONLY`, `SUPERSEDED`, `REJECTED`, `RETIRED`, or a later controlling adjudication may demote the accepted analysis for continuation. Preserve earlier material as provenance when later semantics supersede it.
