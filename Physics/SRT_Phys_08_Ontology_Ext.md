...
> **身份说明**：本条只保留为 V3.0 的 historical / exploratory constitutive model，而非逻辑公理或现行 phenomenality owner。“现象基质”与棱镜语言不建立 qualia、consciousness 或 subjecthood，也不解决涌现问题；$L_0$ 仅按可显现潜能的模型语言读取。
...
泛心论试图赋予物质以意识属性（内在本质）。SRT 不从普遍选择或状态过滤推出 consciousness；把意识称作“状态过滤器”的旧 identity 读法已 superseded。$\hat{G}$ 只在具名模型中表示路径实例化，不承担意识准入。
...
**$L_2$ 候选模型补充**：“大脑 = $\hat{G}_θ$ 的 $L_2$”只保留为 P3/P4 bridge shorthand，不是 canonical identity。该模型可把脑损伤表示为部分沉积结构的物理破坏，并提出以下候选解释：
...
**注意**：此处的批判对象是“将 $L_2$ 等同于全部实在”的还原论。脑动力学可由神经科学描述；把大脑整体简写为 $\hat{G}_\theta$ 的 $L_2$ 只是 bridge model，不能预设 consciousness 的来源、完备解释或外部“广播内容”。

## 1.3 非力学因果公理

**候选物理接口**：本节不预设意识产生“力”。概率重加权／不确定性降低只作为选择因果的 P3/P4 模型，不等同 consciousness-to-matter causation。
...
**与泛心论的决定性区别**：
泛心论试图赋予物质以意识属性（内在本质）。SRT 不从普遍选择或状态过滤推出意识；“意识是状态过滤器”的旧 identity 读法已 superseded。$\hat{G}$ 只在具名模型中表示路径实例化。

**模型边界**：信息选择与能量守恒的兼容性不证明“意识干预”；Landauer 类代价也不建立 phenomenality 或 agency。
...
- **$\hat{G}_\theta$ 的棱镜作用**：在该 exploratory model 中，算子把 $L_0$ 候选折射为 $L_1$ 表征，不同 $\theta$ 产生不同“色散模式”。这不把 $L_1$ 定义为体验；脑损伤相关差异可检验参数模型，但不能证明意识未被改变或消灭。
- **$\Psi_f$ 与存在惯性**: 本体论摩擦决定锚定代价；Higgs 机制与 $\Psi_f$ 在"质量即存在惯性"层面存在**结构类比**（⟶struct，非同构），两者量纲与机制独立，不可直接等同。
- **$d$ 与泛经验场访问**：`d` 可在该 P3/P4 模型中追踪候选内容多样性；它不决定 phenomenality，`d \to \infty` 也不推出全域经验。所谓坍缩故障只作为待检验的动力学分类，不是临床或意识判决。
