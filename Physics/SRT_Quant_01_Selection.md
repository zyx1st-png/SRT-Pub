...
### T-Sel-3 [T1.3.3]: Probabilistic Bias Candidate (概率流偏置候选)
在该 P3/P4 物理模型中，高阶 $\hat{G}$ 可被表示为对 $L_0$ 概率流的微弱偏置；`高阶 G = 意识` 的旧读法已 superseded：
$$ P_{obs}(x) = P_{Born}(x) + δ_θ(x, d) $$
$$ \text{subject to: } \int δ_θ \, dx = 0 \quad (\text{概率守恒}) $$
*   **Critical Constraint**: 对于宏观物体 ($N \sim 10^{23}$)，退相干极速导致 $δ_θ \to 0$。该式不建立 consciousness、phenomenality 或 mind-to-matter causation。
