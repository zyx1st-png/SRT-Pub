---
id: SRT-BOOK-CH06-ORDER-RECOVERED-NOTES
...
# Chapter 6 · Order as Background · Recovered Notes

> Purpose: record source motifs, guardrails, and deferred material for chapter 6 `06_秩序如何成为背景.md`.

---

## 1. Chapter task

Chapter 6 completes the first major chain of Part 1:

```text
L0 → ε_pg → selection → anchoring → order / L2
```

It must shift from mostly defensive ground-clearing to positive construction:

- define order as connected, inheritable, shareable, backgrounded anchoring;
- show order as habitable ground, not merely constraint;
- introduce L2 as second-layer stability without turning this into the full L2/lethal-L2 chapter;
- prepare the next chapter: L2 as condition for later selection.

---

## 2. Style constraints applied

### 2.1 Avoided mechanical “上一章留下的问题” opening

The chapter opens with a morning route scene instead of a direct analytic handoff.
...
## 3. Source motifs recovered from repository context
...
Chapter 6 therefore makes order first a habitable background before analyzing its danger.

### 3.3 From L2 / institution files

Recovered motifs:

- institution as L2 attractor / stable background;
- path dependence;
- rights as protection of selection channels;
- power as control over legitimacy background;
- institutions are not merely texts but enacted, expected, budgeted, enforced patterns.

The chapter keeps these motifs in book-layer language and avoids formulas.

### 3.4 From earlier founder intuitions
...
- κ_mask / κ_relax contrast;
- lethal L2 criterion;
- actual vs felt Ψ_f split;
- replacement L2 vs auxiliary L2;
- A/B occlusion phases.
...
- consequence return matrix;
- collective externalization;
- co-selection reality criterion;
- aggregation / domination / capture degenerations.
...
- L0 pre-pruning by AI;
- high-capability / low-stake deployment;
- S0-S4 spectrum;
- checkpoint vs irreversibility;
- proxy + delegation as modern order.
...
- rights as protective L2 boundary;
...
- Do not make “order” synonymous with “institution”.
