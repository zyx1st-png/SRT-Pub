---
id: SRT-MATERIAL-RERUN-AUDIT-2026-04
type: governance
tags: [Pipeline1, MaterialRerun, Audit, Backfill]
status: active_v1
layer: meta
epistemic_layer: os
claim_mode: canonical
dependency: [SRT-MATERIAL-LOG, SRT-EXECUTION-PLAN]
---

# Pipeline 1 Full Rerun Audit (2026-04-02)
...
- `RR5`：台账已按五问回填，正文也已做更自然的人类可读重写
- `RR4`：正文已补“真正改变了什么”的自然引导句，但台账仍未完全回填
- `RR3`：材料已融入，但正文仍偏 `patch / interface` 体；后续应按台账回推优化
- `RR2`：`B` 观察项或 `C` 非融入项；正文不必改，但台账可按五问补强
- `RR1`：重复提交 / 档案类条目；主要价值是加固增量门与边界门，不要求正文重写
...
| Batch | Line Range | Dominant Material Type | Current Rerun State | Recommended Next Action |
|---|---:|---|---|---|
| Batch A | `45-84` | 早期 interface / encyclopedia / essay 波次 | 多为 `RR3` | 先做台账五问回填，再择要重写高传播正文 |
| Batch B | `85-124` | 中期 science / methods / interview 融合波次 | `RR3` 与少量 `RR2` 混合 | 优先处理 neuroscience / cosmology / AI 主链正文 |
| Batch C | `125-162` | 晚期重复提交、观察项、late paper 波次 | `RR1 / RR2 / RR3` 混合 | 先清理 duplicate / observation，再回推关键 A 类正文 |
| Batch D | `163-166` | 新流程样板条目 | `RR5` | 作为后续改写模板 |

## 4. Observation Queue (`B`)

这些条目不要求正文改写，但应在后续台账回填中明确其 `残余压力` 与重评条件：

- `98`：Popular Mechanics / primordial quantum multiverse
- `100`：New Scientist / cosmology vibe shift
- `107`：Zenodo / Natural Criticality Hypothesis of Subjective Time
- `115`：space / spacetime quasicrystal 讲解稿
- `156`：Popular Mechanics / neutrino gravity universe

统一口径建议：

- `新增接口`：保持候选问题意识，不直接写入正文
- `反向修正`：提醒 SRT 不宜抢跑到过强本体结论
- `加固内容`：通常加固的是方法门与证据门，而非正文命题
- `SRT反哺`：可整理为待验证窗口或实验/文献追踪问题
- `残余压力`：把重评条件写成“需要什么一手锚点 / 同行评审 / 多探针一致性”

## 5. Duplicate / Archive Queue (`RR1`)

这些条目本质上不要求正文重写，主要用于加固增量门：

- `59`：Social Identity 重复提交
- `73`：LoTH 重复粘贴
- `75`：Ritual 重复粘贴
- `79`：Cognitive Ontology 重复粘贴
- `80`：Theory of Mind 重复粘贴
- `125`：Quanta fractal spacetime 再次提交
- `128`：Frontiers Editorial 重复提交
- `145`：ALIFE 中文归档文件
- `157`：Quanta string theory 再次提交
- `159`：Popular Mechanics consciousness 重复提交
- `160`：Quanta astrocytes 再次提交
...
- `加固内容`：既有接口已足以承重
- `SRT反哺`：最多作为增量门回检样本
- `残余压力`：若重复提交来自更强一手来源，再重开审核

## 6. High-Priority Doc Rerun Queue
...
## 7. Working Rule for the Full Rerun

全量重跑不等于“一次性重写所有正文”，而是按两层推进：

1. **台账层全量回填**
   - 把每条材料至少收进 `新增接口 / 反向修正 / 加固内容 / SRT反哺 / 残余压力`
2. **正文层重点回推**
   - 只优先改那些已经形成主链承重、但仍明显停留在 patch 体的落点
