...
| gating (organization-level) | gate, valve, selective permeability, Maxwell-demon gate, support | a formed organization as the actualization of the Selection-operator (`Ĝ`) role; primitive Selection is not gating of a pregiven flow | 2026-09-24 pre-object vertical source §6 + second adjudication D-1; reconciliation plan PV-K1 | author-source; canonical wording pending | PARTIAL_OVERLAP (with `support`) |
