...
| Position-relative L0-facing | formed-position L0-facing, current Position open face | working phrase for inspecting the open / non-preclosed aspect relative to an already formed Position; narrower context than universal L0 aspect; no independent ontology admitted | PR1069 corrective + Facing method v0.2; Spine §§5–6 | working label only | WORKING_LABEL_ONLY / do not harden without residual |
| L0-facing continuity | generative continuity, renewed generativity | **FRH-0 reduction target**: first test whether burden is fully absorbed by O0 non-preclosure + Selection-mediated recurrent reconstitution / One-side history + formed Selection-position; primitive Selection is source of renewed generativity, not by itself sufficient for this-One continuation | PR1069 corrective + Facing method v0.2 | working reduction target | WORKING_LABEL_ONLY; RETIRE if fully absorbed |
| formed Orientation | Position orientation, future-facing Position | working candidate for the prospective non-flat expression of an already formed Position; must remain distinct from `Oriented Openness` / O0 and from basal structural generative expectation B; possible overlap only with C-level anticipatory realization remains to be tested | PR1069 corrective; Expectation Typing 2026-09-20; Spine §6.3 | working label / OPEN overlap | WORKING_LABEL_ONLY / PARTIAL_OVERLAP |
