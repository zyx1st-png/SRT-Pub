---
id: SRT-AUDIT-AGING-SELECTIVE-MAINTENANCE-2026-08-17
type: theory_audit
status: active
...
canonical: false
created: 2026-08-17
governing_protocol: Governance/SRT_GOV_SUB01_Subtractive_Audit_Protocol_v0_1.md
target_bridge: 03_Bridges/SRT_Aging_Selective_Maintenance_Bridge_2026-08-17.md
source_ids:
...
本审计按 `GOV-SUB01` 执行。分类只相对于本次冻结的 target、context、horizon、perturbation 与 replacement budget；不得把 `R2/R3` 读成“该现象不存在”，也不得把暂时留下的残余读成 primitive / fundamental。
...
explain all aging
explain lifespan
explain all hallmarks
prove SRT ontology
prove information theory of aging false
```

### Context `C`

- **bearer**：优先固定到具体 tissue / stem-cell population / organism，实验前必须声明；
- **scale / grain**：cell / clone / tissue / organism 不得事后混用；
- **boundary**：维护成本最终落到哪个边界内必须预先声明；
- **history**：不同的 repair / elimination / replacement / inflammatory / compensatory history；
- **measurement regime**：current burden + reserve + resilience + path-topology endpoints；
- **domain**：aging biology / regeneration / tissue maintenance；
- **time horizon**：至少覆盖恢复后再挑战，必要时第三次 challenge 观察二次 writeback。

### Horizon `H`

```text
formation perturbation
-> recovery / endpoint matching
-> standardized multi-route rechallenge
-> optional third challenge
```
...
1. ordinary repair-dominant episode;
2. elimination / replacement-dominant episode;
3. hormetic / adaptive stress case;
4. chronic inflammatory / high-burden case;
5. a route-blockade or switching challenge;
6. if feasible, a high-regeneration or negligible-senescence counterexample domain.
...
X2 ROS / inflammation / misfolding as Psi_f identity
X3 maintenance has finite cost
...
X11 Maintenance-History Topology Hypothesis
X12 marker reset != generative rejuvenation
```

---

## 3. Single-removal and replacement results
...
## 4. Competitor-specific subtraction
...
```text
with burden and maintenance resources comparable,
does maintenance-route history still alter later route accessibility?
```

### 4.2 Damage / oxidative / proteostasis accounts

Delete ontological-friction wording and retain direct damage variables.

Result: ROS, DNA damage, proteotoxic burden and mitochondrial dysfunction remain fully describable.

**Classification:** `R1/R3` for proxy / vocabulary claims.

Required guard:

```text
biological stressor != Psi_f
```
...
**Classification:** `R2` for simple recovery / rechallenge claims.

Residual question must concern **path repertoire / accessibility / switching**, not only recovery speed.
...
```text
nominal reserve / redundancy / component reliability matched
...
SRT may only ask about the **realized within-lifetime path history**, not why evolution retained the mechanism.

### 4.6 Misrepair-Accumulation — strongest deletion pressure

Delete `payment != undo`, residual-friction and “repair eats future” language; retain tissue Misrepair accumulation.

Result: persistent structural consequence after repair remains strongly expressible.

**Classification:** `R2` for residual-repair history.

Therefore the bridge must not use any of the following as novelty claims:

```text
repair is imperfect
repair leaves structure
fibrosis accumulates
maintenance can have future cost
```

Surviving pressure only exists if **locally appropriate / successful maintenance with low measured misrepair** still changes later path topology.
...
**Current classification:** `R4 compensated / unresolved` until a frozen rival and prospective test exist.

---

## 5. Joint-removal interactions
...
## 6. Perturbation sweep / counterexamples

### Hormesis

Required outcome space:

```text
short-term burden increases
future adaptive capacity increases
```

If the bridge equates higher burden with aging, it fails immediately.

### High regeneration

If repeated tissue loss can be repaired without progressive accessibility loss, “selective elimination -> aging” is false as a general rule.

### Negligible senescence

Long-lived systems with sustained maintenance challenge but weak age-related decline pressure any simple cumulative-friction account.
...
A local `N1` requires that maintenance history or its SRT-specified path/bearer structure yields prospective predictive / intervention gain **after** the bounded rivals are fitted under the same information budget.

### Negative condition

If rival-equivalent performance is obtained, classify:

```text
Maintenance-History Topology Hypothesis -> R2/R3 for this target
SRT ageing bridge -> translation / orientation only
```

No patch multiplication is allowed to manufacture residual novelty.

---

## 8. Relation to current selection-event audit
...
what was damaged?
what was repaired?
what was eliminated / replaced?
what lineage / tissue paid?
what changed later?
```

### Step B — SRT event audit second

Only then ask whether one bounded chain reaches:
...
Failure at any gate means the case cannot be assembled into a bounded selective-maintenance event candidate by compensation from other gates.

---

## 9. Residue classification summary

### `R0-R3` — do not carry as novelty

```text
aging = high friction
ROS / inflammation = Psi_f
maintenance is finite-cost
maintenance = selection
repair leaves residue
short-term protection can have long-term cost
reserve loss = aging
simple recovery-rechallenge difference
marker reset != functional rejuvenation
```

### `R4` — retain only provisionally
...
This audit authorizes only one next scientific step:

> **freeze a bounded competitor matrix and design a multi-route maintenance-history discrimination test.**
