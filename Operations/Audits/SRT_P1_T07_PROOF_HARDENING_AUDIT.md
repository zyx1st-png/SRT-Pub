...
5. **What can P1-T07 prove at most?** Two things must be separated. **(i) True and derivable**: *if* `τ<∞`, then `∅` is absorbing and `P` is no longer a stable ISP (termination just means selection has stopped). **(ii) NOT derivable as written**: *neutral `P` terminates a.s., therefore neutral `P` is not stable* — because a.s. termination of a neutral process is exactly what Step 3 fails to establish. "Positive termination probability" is also **not** unconditional (it needs positive hazard at a surviving step). (§4, §8)
