...
|---|---|---:|---:|---:|
| `SRT_CONTEXT_BUNDLE_SPINE.md` | 骨架 spine | 16 | 380,542 | ~153,695 |
...
|---|---|---:|---:|---|
| 骨架路线（裁定定义时用） | `SPINE` | 153,695 | 46,305 | 需要确定 SRT 术语、公理、方程、符号含义时，只装这一个。 |
| 轻量跨域 | `COMPACTCORE` | 73,182 | 126,818 | 只需领域主线、不需裁定定义时用。 |
...
| 组合 | 合计 ≈token | 为什么禁止 |
|---|---:|---|
| SPINE + COMPACTCORE | **226,877** | 旧版曾把它推荐为跨域方案；两包合计已**超出**整个窗口，装不下。 |

### `SPINE` + 各领域包（逐个列出，均不推荐）

| 组合 | 合计 ≈token | 是否在预算内 |
|---|---:|:---:|
| `SPINE` + `DOMAIN_AI` | 180,968 | **超预算** |
| `SPINE` + `DOMAIN_PHYSICS` | 188,251 | **超预算** |
| `SPINE` + `DOMAIN_PHILOSOPHY` | 188,982 | **超预算** |
| `SPINE` + `DOMAIN_NEUROSCIENCE` | 195,342 | **超预算** |
| `SPINE` + `DOMAIN_SPIRITUALITY` | 167,805 | **超预算** |
| `SPINE` + `DOMAIN_CORE` | 163,380 | **超预算** |
