...
created: 2026-07-09
updated: 2026-07-09
provenance: 2026-07-09 第一直觉 choice-trace 回写实践复盘。该对话本体是普通 LLM 对话（未走任何管线），仅在结束后用选择地图格式回写成 trace。本文件把这一被验证的用法正式化，并修补该实践暴露的四类缺口。v2（2026-07-09）：吸收第二份 trace（去同步化再同步）的实战经验——新增一致性压测协议（§4a，修复 assistant 对作者选择服从性过强、不揭示内外部张力的问题）、暂停/恢复协议（§2a）、第五确认状态、收尾管线张力审计（§5.0）与粘贴即用对话提示词（§7）。
...
4. 定义 trace 的**收尾管线**（张力审计 → canonical 碰撞检查 → 术语撞车检查 → 路由 → 落库）。
...
- 不替代文章工作流：文章类 trace 仍按 `_SRT_CHOICE_TRACE_LOG.md` 的字段与纪律执行，本文件只补 `trace_type` / 回写 / 越界 / 委托 / 张力五块公共协议；
