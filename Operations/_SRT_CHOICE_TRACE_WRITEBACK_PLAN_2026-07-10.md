...
| 3 | 原计划 Q20 遮蔽 + Q22 方向；**实际仅落 Q22**（Q20 未触碰） | 操作顺序主张：1432 循环（去遮蔽→解笼→重建尺度→重分配→新一轮去遮蔽）——原计划 Q20 承接"去遮蔽为什么是第一步"，Q22 §3 权力节承接"垄断如何按此顺序拆"；执行时判断该主张作为跨章交叉引用整段放入 Q22 章末注七即可自洽（引用 Q19/Q20 而不改动 Q20 正文），未在 Q20 内新增内容 | trace2 P2-10 | 无 | 中（Q22 1 段 + 交叉引用；Q20 无改动） |
| 4 | 原计划 Q28 回到生成（+ 幕前·五轻触）；**实际仅落 Q28**（幕前·五未触碰） | 负向链对位段：「没有选择=没有生成=存在退化→脆弱性提升→熵」作为正向主梁"把选择交还给主体的地面"的暗面收口；幕前·五页面极短且节奏紧凑，执行时判断不必要单独触碰，其素材由 Q28 章末注七承接（见 `BOOK_VERSION_LOG.md` 2026-07-10 条目） | trace1 P11/P12/P13 | 无 | 中（Q28 1 段；幕前·五无改动） |
...
| **Phase 1**（已执行） | 理论侧：T-A OPEN_TENSIONS 登记、T-F 候选登记（证据卡本身未开始，见 T-F 二级路径）、T-G 政治哲学对表；书稿侧：B-A #1–4 直接改章（Q06 边界收口、Q19 四分层、**实际仅落 Q22 的** 1432 循环、**实际仅落 Q28 的** 负向链）+ #5 熵半边章末注（并入 Q28，见 B-A #4/#5 说明）+ #8 术语表随动（未补词条，见下方"未执行项"） | 无门 |
