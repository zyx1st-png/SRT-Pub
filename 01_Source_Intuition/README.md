# 01 Source Intuition / 源头直觉

This layer restores the founding force of SRT before formalization, bridge work, public packaging, or governance.

SRT does not begin as a documentation system. It begins from a reversal:

> Reality is not first given and then selected. Reality becomes determinate through constrained selection.
>
> 现实不是先被给定、再被选择；现实是在约束中的选择里变得确定。

## Role

`01_Source_Intuition` is the frontstage home for SRT's founding intuition, philosophical essays, non-formal descriptions, and source-level orientation.

It should answer:

- Why does SRT begin with selection rather than existence?
- Why is attention only the cognitive surface of a deeper selection process?
- Why is the subject not the precondition of selection, but one of its stabilized products?
- Why does value require stake, consequence return, and non-substitutability?
- Why is SRT not idealism, relativism, or panpsychism?

## Core thesis

SRT's founding intuition can be stated as:

> Existence is not a static inventory of objects. It is the stabilized trace of selection under constraint, cost, concern, and history.

More compactly:

> Existence is selected, anchored, paid for, and hardened.

## Current source anchors

The current repository already contains source-level files. Until a later migration pass, this layer points to them rather than moving them.

- [`../Core_Law/SRT_L0_Metaphysics.md`](../Core_Law/SRT_L0_Metaphysics.md) — unique L0 metaphysical anchor.
- [`../Core_Law/SRT_Selection_Argument.md`](../Core_Law/SRT_Selection_Argument.md) — positive argument and objection handling for selection before existence.
- [`../Core_Law/SRT_Core_Text_CN_Euclid.md`](../Core_Law/SRT_Core_Text_CN_Euclid.md) — Chinese core-text candidate.
- [`../Core_Law/SRT_Core_Text_CN.md`](../Core_Law/SRT_Core_Text_CN.md) — historical Chinese core-text entry.
- [`../SRT_Quick_Start.md`](../SRT_Quick_Start.md) — introductory reader path.

## Writing mode

This layer should stay sharp, direct, and non-bureaucratic.

Preferred style:

- philosophical but readable;
- source-level rather than defensive;
- conceptually bold but not careless;
- no premature equations;
- no governance-first framing;
- no claim that public intuition replaces formal anchors.

## Boundary

This layer is not a substitute for canonical definitions. It is the source of orientation and force. Formal definitions belong in `02_Formal_Anchors`; external theory translation belongs in `03_Bridges`; evidence grading belongs in `04_External_Convergence`; public-facing release belongs in `05_Public_Release`.
