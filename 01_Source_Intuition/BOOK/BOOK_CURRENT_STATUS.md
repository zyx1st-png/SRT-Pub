...
- Stage 5 首尾与贯通【进行中：**致读者改五幕**（v6）+ **全书 act 引用一致性顺读·第一批**（Q22 §1 旧四幕口径 + 附录术语表「第四幕 秩序与边界」拆成第四幕「从摩擦到秩序」/第五幕「从秩序回到生成」）已落；+ **幕终·四落地**（`幕间桥_四五幕.md`，Q22 与幕前·五之间）已落；剩 幕前/幕终·一 + 两座幕内幕间桥归位 + Q23–Q28 frontmatter 第四幕→第五幕改标（含 position 字段，机械 PR）+ 章号 renumber（选材/能动性插入致总数变化）+ Q10/Q11 独立关系复核 + 全书顺读余下批次】：幕前/幕终·一 + 致读者改五幕 + 全书顺读
