...
- Stage 5 首尾与贯通【进行中：**致读者改五幕**（v6）+ **全书 act 引用一致性顺读·第一批**（Q22 §1 旧四幕口径 + 附录术语表「第四幕 秩序与边界」拆成第四幕「从摩擦到秩序」/第五幕「从秩序回到生成」）已落；+ **幕终·四落地**（`幕间桥_四五幕.md`，Q22 与幕前·五之间）已落 + **幕前·一取消**（序章 Q00 即第一幕开门页、已 enact 幕前四件事；幕终·一＝既有幕间桥_一二幕；第一幕首尾槽位已满）；剩 两座幕内幕间桥归位 + Q23–Q28 frontmatter 第四幕→第五幕改标（含 position 字段，机械 PR）+ 章号 renumber（选材/能动性插入致总数变化）+ Q10/Q11 独立关系复核 + 全书顺读余下批次】：致读者改五幕 + 幕终·四 + 全书顺读
