...
净变化（2026-06-24 修订；2026-06-25 续）：新章 4（选材 / 能动性 / 本体论摩擦 / 权力）——全景章取消（幕前·三吸收）；意义并入价值章；世界图景靠各章内容优化、不立结构；**Q10/Q11 保持独立、不合并**；~~旧 Q26 可证伪性下沉 Q27 + 附录~~（**2026-06-25 取消：Q26/Q27 保持独立、不折入，见五幕骨架修订记录**）。
...
| 编号 | 文件 | 当前状态 |
|---|---|---|
| 致读者 | `Drafts_26Q/致读者.md` | `draft_v6`（v6 五幕重构·Stage 5：四幕地图→**五幕运动**（拆地板→选材/成地板→长出世界→摩擦/秩序→回到生成）；新增「幕前与幕间」段（幕前从第二幕起开门提大问题、幕间回望）；删去旧幕间清单里失效的「权力/自我证明」、按运动改写不再钉死「第X至Y章」区间（章号待 Stage 5 renumber）。早前 v4–v5 从略） |
...
| 幕前·五 | `Drafts_26Q/幕前_五幕.md` | `draft_v1`（五幕重构新增·第五幕幕前：提出“人被什么连成共同体／做选择承受后果是否必须有意识／历史有没有一个终点·秩序或理论建成后是否只需维护”传统大问题、指出旧问法（历史有终点抵达即完成、只有有意识个体才算数、一套对的理论就是不容质疑的源头）都默认“完成态”+把待交还选择的主体预设成现成清醒个体、预告“多主体存在（共同体）→AI 改写选择条件→选择是否需要意识→理论自反/失败条件→回到生成”运动；不讲机制。**真章勘察：建筑图「吸收旧幕间五」不成立（幕间桥_Q24_Q25 在幕内），故独立新增、不移动幕间桥、不改 Q23–Q28 frontmatter；幕终·四+幕间桥归位留 Stage 5**） |
...
| Q22 | `Drafts_26Q/Q22_方向.md` | `draft_v27`（v27 五幕重构·Stage 5 顺读：§1 旧四幕口径「第四幕前半段（Q18–Q21）」改五幕「这第四幕…Q18 起直到本章」；「前四幕」保留=acts1–4 of 5 已正确。v26 §4 第二问外包线轻收拢——显式接 §3 权力＋本体论摩擦（外包＝某个更有力量的位置把摩擦悄悄分配给回推不了的位置）；v25 新增 §3「权力：谁在分配摩擦」一节，桥接本体论摩擦→方向三问；早前从略） |
...
- Stage 3 四幕【主体完成（加节/小修为主、非新章）：本体论摩擦＝Q18 §3、权力＝Q22 §3、幕前·四（新增）、Q21/Q22 摩擦/外包线轻收拢 均已落；幕终·四尚未落地，留 Stage 5 贯通时处理/复核（幕间桥_三四幕是幕终·三，不能充当幕终·四）；下一步=转 Stage 4】：本体论摩擦节 + 权力节 + 幕前·四 + Q21/Q22 收拢
- Stage 4 五幕【进行中：幕前·五已落；**Q26 折入 Q27 经真章勘察取消**（45KB+刻意两步，作者拍板「保持独立+轻交还接缝」，已落 Q27 §4「交还机制」段）；剩 多主体/交还重构（Q23–Q28 视角对齐/frontmatter 第四幕→第五幕改标）+ 终章收口 待推】：多主体/交还重构 + ~~Q26 折入 30~~（取消）+ 终章 + 幕前·五
- Stage 5 首尾与贯通【进行中：**致读者改五幕**（v6）+ **全书 act 引用一致性顺读·第一批**（Q22 §1 旧四幕口径 + 附录术语表「第四幕 秩序与边界」拆成第四幕「从摩擦到秩序」/第五幕「从秩序回到生成」）已落；+ **幕终·四落地**（`幕间桥_四五幕.md`，Q22 与幕前·五之间）已落 + **幕前·一取消**（序章 Q00 即第一幕开门页、已 enact 幕前四件事；幕终·一＝既有幕间桥_一二幕；第一幕首尾槽位已满）；+ **Q23–Q28 frontmatter 改标五幕**（`act:` 第四幕→第五幕 + `position_in_seven_stages` 站号重排为第五幕第一…五站/终章；内部元数据、非 reader-facing）已落；**章号/总数 renumber 经勘察暂不做**（361 处「第X章」交叉引用 + ~20 处复合引用如「第十四到十六章」跨插入点端点位移不同；书内现状已自洽=每个第X章指向正确章节、选材/能动性作主题标题不编号；作者拍板暂不 renumber，留最终排版时带校验专项）；+ **两座幕内幕间桥 frontmatter 归位**（Q23_Q24/Q24_Q25：id ACT4→ACT5-INNER、role 加【五幕定位】标为幕内非幕终）已落；剩 Q10/Q11 独立关系复核（预期零改动）+ 全书顺读余下批次（预期收益小）；章号 renumber 已决策留排版期】：致读者改五幕 + 幕终·四 + 全书顺读
