...
2. **基础形成问题 cycle**：围绕 multiplicity / Selection / One-formation 恢复作者直觉并接受 strongest-neighbor pressure；当前 `world-side determinate manifest actuality requires Selection` 与 `subjectless Selection` 都是 K，`Selection <-> broader One-formation` 精确关系才是 D。
...
6. **Non-substitutability test**：任何 candidate SRT response 必须给出对具名 strongest neighbor 的 Level 0/1/2 verdict，并记录其 source-native claim、为何是 strongest available comparator、SRT 改变的具体 structural item；Level 0 不得冒充 substantive SRT response。
...
- L0 对齐恢复：world-side determinate manifest actuality 依赖 Selection、subjectless Selection、Order 非 L0 原生承诺与旧 order-drive rollback 均继续为当前 K；开放的是 broader One-formation / stabilization / objectification / later order-convergence 的更深关系，而不是重新把 K 置为对称候选。治理摘要全部指回 canonical owner locator，owner 与摘要冲突时 owner controls。
- 理论形成顺序保持 author re-entry → neighbor adaptation → common-problem extraction → author second adjudication；同时加入 Level 0/1/2 non-substitutability / structural-consequence gate，并要求 Level verdict 绑定具名 strongest neighbor，堵住挑弱 comparator 的 gaming path。
