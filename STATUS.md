...
- **2026-08-16 selection irreducibility GOV-SUB01 Pass 1 已执行 K-budget 分解，但正式 residue 仍为 `UNASSIGNED`。** `K=0` 显示当前 grammar 依赖；limited/broad K 显示历史不对称、bearer consequence 与部分 event-classification 工作存在较强可重构性；primitive actualisation 仍受 hidden-reparameterization / target-circularity 双重压力。由于 §13 要求的 `E_cf / E_int / divergence experiment / E_phen-E_norm` exhibit 均未产生，任何 `R* / N* / P` 只能作为 candidate reading。Step 5 joint removal 与扰动 sweep 尚未执行；下一门为最低成本 Pass 1.5 联删后进入 A2 rival adjudication。记录：`Operations/Audits/SRT_SELECTION_IRREDUCIBILITY_RIVAL_TEST_2026-08-16.md`。
