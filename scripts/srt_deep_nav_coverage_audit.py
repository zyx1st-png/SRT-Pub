...
Run from repository root:
    ...
    python scripts/srt_deep_nav_coverage_audit.py
...
def collect_referenced_paths(root: Path, nav_files: list[str]) -> set[str]:
    refs: set[str] = set()
...
def classify_priority(path: str, referenced: bool) -> tuple[str, str]:
    if referenced:
        return "referenced", "already referenced by navigation layer"
...
def build_coverage(files: set[str], refs: set[str]) -> list[FileCoverage]:
    coverage: list[FileCoverage] = []
    for f in sorted(files):
        # A file can be referenced with or without leading ./; refs are normalized.
        referenced = f in refs
        priority, reason = classify_priority(f, referenced)
        coverage.append(FileCoverage(f, top_dir(f), referenced, priority, reason))
    return coverage
...
def build_markdown_report(coverage: list[FileCoverage], refs: set[str]) -> str:
...
def main() -> int:
    ...
    refs = collect_referenced_paths(root, nav_files)
    coverage = build_coverage(files, refs)
    report = build_markdown_report(coverage, refs)
