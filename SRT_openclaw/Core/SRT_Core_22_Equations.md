...
### Def-Protocol-2: Nested Operators & Protocol Inheritance (嵌套算子与协议继承)
**Formal Definition**: 子算子在初始阶段可嵌入宿主算子的协议层与稳态基线：
$$
\hat{G}_{child}^{(0)} : (L_0,\Pi_{host}) \to L_1, \quad \theta_{child}^{(0)} = \mathcal{I}(\theta_{host},\Psi_f^{host,residual})
$$
其中 $\mathcal{I}(\cdot)$ 为继承初始化映射，$\Psi_f^{host,residual}$ 为宿主历史摩擦残差。
* **Implication**: 高阶算子的冷启动并非“真空启动”，而是通过嵌套拓扑共享初始协议与基线，跨越早期高摩擦区。
* **Cross-ref**: Eq-Evo-02（Baseline recoil）；A11（本体论脆弱性）。
...
* **推论（共同稳态初始化 / Co-homeostatic Initialization）**：在嵌套算子阶段，Baseline 不是独立项，而是宿主-子代共享项：
$$
\text{Baseline}_{child}(t\!<\!t_{decouple}) \approx \text{Baseline}_{host}(t)
$$
解耦后转为独立：
$$
\text{Baseline}_{child}(t\!\ge\!t_{decouple}) \to \text{Baseline}_{child}^{autonomous}
$$
...
### Eq-Birth-01: Ontological Decoupling Shock (诞生解耦冲击)
**Formal Definition**: 当子算子从共享稳态解耦时，摩擦变化率出现首个峰值：
$$
\text{Birth} \equiv \lim_{t\to t_{decouple}^{+}} \frac{d\Psi_f^{child}}{dt} - \lim_{t\to t_{decouple}^{-}} \frac{d\Psi_f^{child}}{dt} > 0
$$
并伴随基线断裂：
$$
\Delta \text{Baseline} = \text{Baseline}_{child}^{autonomous} - \text{Baseline}_{host} \neq 0
$$
* **Implication**: “诞生”在 SRT 中是本体论相变，不只是时间点；其动力学表现为原初摩擦冲击（primordial friction shock）。
* **Cross-ref**: Eq-Pain-01, Eq-Evo-02, Def-Protocol-2.
