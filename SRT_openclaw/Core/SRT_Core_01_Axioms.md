...
- **反证阈值（硬门）**：定义速度-精度偏移指数 `S=z(RT)-z(Accuracy)`，并比较 `ΔS=S_high_risk-S_low_risk`。若出现 `ΔS≤0`（高风险不出现速度优先）或 `ΔS<0` 且跨样本稳定（高风险反而更精度优先），且 HRV/EDA 证明高风险操纵有效，则触发 A7 硬门反证。
- **降级规则（严格口径 A）**：触发硬门并完成两轮独立复现后，A7 从“优先级主张”降级为“情境性命题”。
...
生存不是静态标签，而是系统在持续调节中维持于可持续吸引域。

$$
\mathrm{Life}(t)\equiv \mathbf{1}\{\sigma_t\in \mathcal{A}_{sustain}\}
$$

动态维持由以下优先序驱动（本轮确认）：

$$
\Psi_f^{inter\text{-}G} \;>\; C_{boundary} \;>\; E_{in}
$$

即：算子间摩擦调节优先，其次边界承载，最后外部通量输入。

#### 8.2 直观解释

- “活着”不是停在高概率点，而是通过耗散与反馈，使轨迹持续留在可维持域；
- 死亡可被视为调节失效导致轨迹脱离 \(\mathcal{A}_{sustain}\)。

#### 8.3 可证伪预测

- 若在控制 \(\Psi_f^{inter\text{-}G}\) 与 \(C_{boundary}\) 后，生存维持仅由 \(E_{in}\) 决定且可复现，则本优先序需修订。
- 反之，外部冲击下若 \(\Psi_f^{inter\text{-}G}\) 与 \(C_{boundary}\) 对维持结果具有主导解释力，则支持 A8 动态版。

#### 8.4 最小实验入口（MVP）

- **任务范式**：分阶段外扰 + 恢复任务，分别操纵算子间耦合、边界缓冲、输入通量。
- **核心指标**：
  - 维持指标：失稳概率、恢复时间、漂移率；
  - 结构指标：边界承载冗余、耦合摩擦强度 proxy。
- **支持 A8 的模式**：在同等输入通量下，\(\Psi_f^{inter\text{-}G}\) 与 \(C_{boundary}\) 的变化可显著改变维持结果。
- **反证阈值（示例）**：若跨任务和跨样本均无法观察到上述主导关系，A8 动态定域版本需降级。
...
### A7-A12 冲突裁决条款（理论优先版，新增）

当 A7（适应度优先）与 A12（跨尺度结构一致）在同一推导步发生冲突时，采用**生存优先**：

1. 先保留 A7 的局部可行动性与生存收敛；
2. A12 的统一结构主张可暂时降为“后验整理约束”；
3. 待风险窗口结束后，再尝试把局部策略回映射到跨尺度统一语法；
4. 若无法回映射，优先记录为“域限结构分叉”，而非强行维持统一叙述。

该条款仅规定**冲突时的优先级顺序**，不取消 A12 的长期整合作用。

### A10-A11-A8 接口判定流程（新增）

针对“终止/失稳/崩溃”类问题，按固定顺序判定：

1. **A10（层级判定）**：先判断该事件在 \(L_1/L_2\) 的终止语义，禁止越级把判定外推到 \(L_0\)。
2. **A11（外部脆弱性）**：再判断外部冲击与算子间摩擦是否触发脆断条件。
3. **A8（动态维持）**：最后评估系统是否还能通过调节回到可持续吸引域，或已脱离维持域。
