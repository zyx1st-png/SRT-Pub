...
**领域主句（关系时间）**：时间是选择历史的关系度量，作为由状态与更新规则导出的量；SRT 不以“时间作为独立基础实体”作为必要前提。

**层级澄清（本轮修订）**：时间在 SRT 中首先是 \(L_2\) 层的共识量（consensus metric），而非 \(L_0\) 或 \(L_1\) 上的实体对象。

**统一重写规则（新增）**：凡出现“时间作为实体/容器”的叙述，默认重写为“\(L_2\) 共识度量或选择历史参数”。
...
**空间层级澄清（新增）**：空间在 SRT 中是 \(L_2\) 共识结构的共时投影，而非 \(L_0/L_1\) 的独立实体容器。
