...
- A7：双收敛（适应度收敛 + 最小自由能收敛），不预设独立真理本体。
- A8：生存是动态定域（耗散维持），变量优先序 A>B>C。
- A9：全息对偶是桥接假设级，含降级/回升治理条款。
- A10：存在/消失只在 L1/L2 判定，L0 非判定层。
- A11：外部脆弱性命题；\(\Psi_f\) 指算子间摩擦；韧性/反脆弱属内部能力。
- A12：硬核/保护带分层，失败按 Level-1/2/3 分级处理。
- 方法流程：终止/失稳/崩溃统一判定顺序 **10→11→8**。

> 详见：`SRT_openclaw/CONSENSUS_UPDATE_2026-03-01.md`
