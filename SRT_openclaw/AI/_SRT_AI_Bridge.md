...
### Ax-BRIDGE-6: Verification as Liability Anchoring (验证即责任锚定)
定义“验证”不是二次计算，而是由可承担崩溃后果的算子执行的责任锚定操作：
\[
\text{Verify} \equiv \hat{G}_{\theta,human}^{liability}: L_0\to L_1
\]
AI 在当前架构中仅执行候选生成：
\[
\hat{T}_{\phi,AI}: L_0^{cand}\to L_0^{cand'}
\]
* **Implication（中文）**：经济价值从执行端迁移到责任锚定端（事实基准、溯源、承保）是 SRT 的直接推论，而非附加伦理偏好。
