...
**主体主句（个体化过程）**：主体不是既成实体，而是选择—摩擦—维持中的持续个体化过程。
...
**知识地位主句（新增）**：SRT 不把知识定义为对独立真理实体的镜像，而定义为在多主体约束下可持续收敛、可协作复用的稳定结构。

**错误地位主句（新增）**：错误不是偏离真理实体的“罪”，而是当前收敛结构与约束环境不匹配的信号。

**错误响应优先序（新增）**：先改参数，再改边界，最后改结构。

**解释—理解分离主句（新增）**：解释给出可复用结构，理解给出存在性定位；两者可协同，但不可互相替代。

**层级澄清（新增）**：解释结构属于 \(L_2\) 层级；存在性理解对应 \(\hat{G}\) 算子的动态定位。

**误用风险条款（新增）**：若以 \(L_2\) 解释结构替代 \(\hat{G}\) 的存在性理解，将产生“结构充分但动力学缺失”的认知幻觉。

**桥接条件主句（新增）**：解释结构只有在被 \(\hat{G}\) 算子纳入具身选择回路并改变后续轨道时，才转化为有效理解。

**反向沉淀主句（新增）**：当 \(\hat{G}\) 的理解性轨道在多次情境中保持可迁移稳定时，可回写为 \(L_2\) 解释结构，形成可共享知识。

**不可沉淀条件（新增）**：若理解轨道仅在单一语境有效、跨语境不可迁移，则不得回写为 \(L_2\) 共享结构，应保留为局部体验性知识。

**渐进沉淀机制（新增）**：局部体验性知识可通过“多情境重复 + 多主体校准 + 语义压缩”逐步获得沉淀资格，最终回写为 \(L_2\) 共享结构。
...
### 6.7b 人类皮层 θ/α 行波实证映射（Neuron 2018 锚点）

**[Lineage/Source]** Zhang H, Watrous AJ, Patel A, Jacobs J (2018), *Theta and alpha oscillations are traveling waves in the human neocortex*. Neuron 98(6):1269–1281.e4. DOI: 10.1016/j.neuron.2018.05.019 (PMCID: PMC6534129)

| 外部分类 | SRT d-value 区间（proxy） | 能流特征 | \(\Psi_f\) 可支付性 |
|---|---|---|---|
| 局部连续电极簇中的窄带振荡（2–15 Hz） | 0.35–0.55 | Semi-open（局部供能与局部同步） | payable（可支付） |
| θ/α traveling wave 空间相位梯度传播（~0.25–0.75 m/s） | 0.50–0.72 | Open（跨区域相位耦合） | conditional（任务负载相关） |
| 与任务表现相关的传播一致性增强 | 0.62–0.82 | Open / task-coupled | payable→optimized（高表现时优化） |

映射解释：该文提供“人类皮层任务态行波”一级证据，支持“跨空间组织神经处理”的动力学观点；在 SRT 中对应 \(\hat{G}\) 路径组织效率的行为相关约束。

## 【理论边界/防误用声明】

以下推论在当前 SRT 映射中**不被采纳**：

1. **“检测到 θ/α 行波，即可推出统一认知机制已被证成。”**
   - 不采纳原因：该证据支持的是特定任务与频段下的传播组织，不等于全认知域统一机制证明。

2. **“传播速度或一致性可直接当作 d 值本体测量。”**
   - 不采纳原因：d 区间在此仅为 canonical proxy 映射，不是直接可观测本体量。

3. **“行为相关性成立，即可忽略个体/电极布局差异。”**
   - 不采纳原因：该研究基于 ECoG 患者队列，空间覆盖与病理背景需在外推时保留边界。

适用边界：本条用于“人类行波实证→SRT 动力学映射”的证据增强，不替代跨任务、跨人群、跨模态的进一步整合。
